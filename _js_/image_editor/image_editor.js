async function getCameraList(camSelect) {
  ...
  class ImageEditor {
      ...
      constructor(options) {
        ...
        // Relación de aspecto normalizada: ancho / alto. Null significa libre.
        this.ratio = ImageEditor.normalizeRatio(options.ratio);
        this.cropper = null;
      ...
      /**
       * Convierte formatos como "2:3", "2/3", "2x3", "portrait" y
       * "landscape" a un número usable por Cropper y por la captura de cámara.
       */
      static normalizeRatio(ratio) {
        if (ratio === undefined || ratio === null || ratio === '' || ratio === 'free') {
          return null;
        }
        ...
        const parts = value.split(/\s*[:\/]\s*|\s*x\s*/);
        if (parts.length === 2) {
        ...
        console.warn(`Relación de aspecto no válida: ${ratio}`);
        return null;
      }
      ...
      cancelEdition(){
          ...
          setTimeout(function(){
              ...
              ImageEditor.editable_images(id,url,null,'jpeg',this.ratio);
          }.bind(this),2000);
      ...
      ******/
      ...
      async handleFile(file) {   
        ...
        if (file && file.type.startsWith('image/')) {
            try {
                ...
                this.img.src = result;
                this.img.classList.remove('no-image');   // ya hay imagen: fuera el placeholder
                this.setState(this.states.READY_TO_SAVE);
            } catch (error) {
      ...
      initCropper() {
          const aspectRatio = this.ratio === null ? NaN : this.ratio;
      ...
      openCamera() {      
          ...
          videoWrapper.classList.add('image-editor-video-wrapper');
          bottomBar.classList.add('image-editor-bottom-bar');

          // Marco visual del área que se conservará en la captura.
          if (this.ratio !== null) {
            videoFrame.classList.add('image-editor-camera-frame');
          ...
          const updateCameraFrame = () => {
            if (this.ratio === null || !videoFrame) return;
            ...
            let frameHeight = frameWidth / this.ratio;
            if (frameHeight > videoHeight) {
              frameHeight = videoHeight;
              frameWidth = frameHeight * this.ratio;
            }

            videoFrame.style.width = `${frameWidth}px`;
            videoFrame.style.height = `${frameHeight}px`;
            videoFrame.style.left = `${(videoWidth - frameWidth) / 2}px`;
            videoFrame.style.top = `${(videoHeight - frameHeight) / 2}px`;
          };

          video.addEventListener('loadedmetadata', () => requestAnimationFrame(updateCameraFrame));
          video.addEventListener('resize', updateCameraFrame);
          ...
          captureButton.addEventListener('click', () => {
            ...
            // La cámara puede entregar una proporción distinta. Recortamos
            // centrado para obtener la proporción solicitada sin deformar la imagen.
            if (this.ratio !== null && sourceWidth > 0 && sourceHeight > 0) {
              const sourceRatio = sourceWidth / sourceHeight;
              if (sourceRatio > this.ratio) {
                cropWidth = sourceHeight * this.ratio;
                sourceX = (sourceWidth - cropWidth) / 2;
              } else if (sourceRatio < this.ratio) {
                cropHeight = sourceWidth / this.ratio;
      ...
      prepareFormData(blob) {
          ...
          // El nombre base sale de originalSrc, pero puede no haber imagen previa
          // (src acaba en '/') o venir sin extensión: en ese caso, nombre generado.
          const baseName = (this.originalSrc || '').split("?")[0].split("/").pop();
          const nameMatch = baseName.match(/(.+)\.([^.]+)$/);
          const fileName  = nameMatch ? nameMatch[1] : `img_${Date.now()}`;
          // cropped-image
      ...
      handleUploadSuccess(data, timestamp) {
        ...
        if (this.destImg) {
          ...
          setTimeout(function(){
              loader.style.display = 'none';
              console.log('EDITABLE.IMAGE',id,url);  //,src)
              ImageEditor.editable_images(id,url,null,this.imageFormat,this.ratio);
          }.bind(this),1000);
      ...
      static editable_images(selector,ajax_url,onUpload=null,image_format='jpeg',ratio=null){
        ...
        document.querySelectorAll(selector).forEach(image => {
            ...
            // Permite configurar la proporción directamente en el elemento:
            // <img data-ratio="2:3">. El argumento explícito tiene prioridad.
            const image_ratio = ratio !== null && ratio !== undefined
                              ? ratio
