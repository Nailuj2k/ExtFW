// Sobrescribir alert()
    ...
    moveElementToTarget = function(element, target) {
                    ...
                    // OJO: position:absolute se resuelve contra el ancestro posicionado. En themes
                    // con body centrado y position:relative (p.ej. noxtr), usar coordenadas de
                    // documento desplazaba el overlay a la derecha el hueco del centrado. Se
                    // convierte al sistema de coordenadas del offsetParent real.
                    element.overlay.style.position = 'absolute';
                    const offsetParent = element.overlay.offsetParent || document.documentElement;
                    const parentRect = offsetParent.getBoundingClientRect();
                    element.overlay.style.top = (targetRect.top - parentRect.top + 5 + verticalOffset) + 'px';
                    element.overlay.style.left = (targetRect.left - parentRect.left + 5) + 'px';
                    ...
                    // El overlay base centra su contenido (flex justify-content:center), así que
                    // con width auto el notify quedaba como caja estrecha centrada ("a partir del
                    // 50%"). Forzamos que el contenido llene el ancho del target.
                    const contentEl = element.overlay.querySelector('.wq-dialog-content');
                    if (contentEl) {
