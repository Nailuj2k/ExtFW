...
if($_HTML_duplicate_entry??false){
    ...
    echo $_HTML_duplicate_entry;
                ...
                if($row['ALLOW_COMMENTS'])  Comments::show($_ID_); 
                    ...
                    $title = Table::escape(ucfirst(str_replace('_',' ',$_ARGS[1])));
    ...
    if($_ACL->HasPermission('edit_items')){
        ...
        <p><?=t('IF_YOU_WANT_TO_CREATE_IT_CLICK','Si desea crearla pulse')?> <a href="<?=htmlspecialchars($_ARGS[0].'/'.$new_pagename)?>/create" style="color:#da2335;font-weight:bold;"> <?=t('THIS_LINK','éste enlace')?> </a></p>
        ...
        foreach($rows as $row){
            echo '<a href="'.htmlspecialchars($_ARGS[0].'/'.$new_pagename.'/create/'.$row['item_id']).'">'.htmlspecialchars($row['item_title']).'</a><br />';     
        }                
    }else{
