        ...
        **/
        ...
        function onTableEvent(data){
            ...
            if        (data.table=='CLI_PAGES_FILES'||data.table==_TB_PREFIX_ + '_' + _TB_NAME_+'_FILES'){ //}          'NOT_NEWS_FILES'||data.table=='BLG_BLOG_FILES'){
            ...
            }else if  (data.table=='CLI_PAGES' || data.table=='NOT_NEWS'){
                console.log('data',data);
                ...
                let f_url = data.table=='CLI_PAGES' 
                          ? '/control_panel/ajax/op=getfield/field=' + _TEXT_FNAME_ + '/field_alt=' + _TEXT_FNAME_A + '/table='+data.table+'/key=item_id/value='+_ID_
        ...
        $(document).ready(function() {
            ...
            function moveElementTo(element,bottom=50,right=50) {
                ...
                if(console_log) console_log('moveElementTo',newPositionTop,newPositionRight,windowHeight,lineHeight,windowWidth,lineWidth,desiredBottom,desiredRight);
                               ...
                               default:
                                   ...
                                   if(role=='insertImage'){
                                       ...
                                       */
                                       ...
                                       var contentEl = document.getElementById('content_'+_NAME_);
                                       var sel = window.getSelection();
                                       var savedRange = (sel && sel.rangeCount) ? sel.getRangeAt(0).cloneRange() : null;
                                       var aEl = null, _n = sel ? sel.anchorNode : null;
                                       while(_n && _n !== document){
                                           if(_n.nodeType===1 && _n.tagName==='A'){ aEl = _n; break; }
                                           _n = _n.parentNode;
                                       }
                                       var hasSel = !!(savedRange && !savedRange.collapsed);
                                       var esc = function(s){ return String(s==null?'':s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); };
                                       var curUrl     = aEl ? (aEl.getAttribute('href')||'')   : '';
                                       var curTitle   = aEl ? (aEl.getAttribute('title')||'')  : '';
                                       var curTarget  = aEl ? (aEl.getAttribute('target')||'') : '';
                                       var curNofollow= aEl ? /(^|\s)nofollow(\s|$)/.test(aEl.getAttribute('rel')||'') : false;

                                       var form = '<div class="wq-linkform" style="display:flex;flex-direction:column;gap:8px;min-width:340px;padding:14px;">'
                                           + '<label>URL<br><input type="text" id="wqlink-url" style="width:100%;box-sizing:border-box" placeholder="https://" value="'+esc(curUrl||'https://')+'"></label>'
                                           + (!hasSel && !aEl ? '<label>Texto<br><input type="text" id="wqlink-text" style="width:100%;box-sizing:border-box" value=""></label>' : '')
                                           + '<label>Título (tooltip)<br><input type="text" id="wqlink-title" style="width:100%;box-sizing:border-box" value="'+esc(curTitle)+'"></label>'
                                           + '<label>Destino<br><select id="wqlink-target" style="width:100%;box-sizing:border-box">'
                                           +   '<option value=""'+(curTarget===''?' selected':'')+'>Misma ventana</option>'
                                           +   '<option value="_blank"'+(curTarget==='_blank'?' selected':'')+'>Nueva pestaña</option>'
                                           + '</select></label>'
                                           + '<label style="display:flex;align-items:center;gap:6px"><input type="checkbox" id="wqlink-nofollow"'+(curNofollow?' checked':'')+'> rel="nofollow"</label>'
                                           + '</div>';

                                       var closeDlg = function(overlay){
                                           var dc = overlay && overlay.querySelector('.wq-dialog-content');
                                           if(dc && dc._dialogInstance) dc._dialogInstance.close();
                                           else if(overlay && overlay.parentNode) overlay.parentNode.removeChild(overlay);
                                       };
                                       var buildRel = function(nofollow, target){
                                           var parts = [];
                                           if(nofollow) parts.push('nofollow');
                                           if(target==='_blank'){ parts.push('noopener'); parts.push('noreferrer'); }
                                           return parts.join(' ');
                                       };
                                       var setAttr = function(el,name,val){ if(val) el.setAttribute(name,val); else el.removeAttribute(name); };
                                       var unwrap = function(el){
                                           while(el.firstChild) el.parentNode.insertBefore(el.firstChild, el);
                                           el.parentNode.removeChild(el);
                                       };
                                       var apply = function(event, overlay){
                                           var url    = (document.getElementById('wqlink-url').value||'').trim();
                                           var title  = (document.getElementById('wqlink-title').value||'').trim();
                                           var target = document.getElementById('wqlink-target').value;
                                           var nofollow = document.getElementById('wqlink-nofollow').checked;
                                           var textEl = document.getElementById('wqlink-text');
                                           var rel = buildRel(nofollow, target);
                                           if(aEl){
                                               if(url===''){ unwrap(aEl); }
                                               else{
                                                   setAttr(aEl,'href',url); setAttr(aEl,'title',title);
                                                   setAttr(aEl,'target',target); setAttr(aEl,'rel',rel);
                                               }
                                               closeDlg(overlay); return;
                                           }
                                           if(url===''){ closeDlg(overlay); return; }
                                           ...
                                           if(hasSel){
                                               ...
                                               html = '<a '+attrs+'>'+tmp.innerHTML+'</a>';
                                           }else{
                                               var txt = (textEl && textEl.value) ? textEl.value : url;
                                               html = '<a '+attrs+'>'+esc(txt)+'</a>';
                                           }
                                           document.execCommand('insertHTML', false, html);
                                           closeDlg(overlay);
                                       };
                                       var btns = [];
                                       if(aEl) btns.push({ text:'Quitar enlace', class:'btn', action:function(e,ov){ unwrap(aEl); closeDlg(ov); } });
                                       btns.push({ text:'Aceptar',  class:'btn btn-primary', action:apply });
                                       btns.push({ text:'Cancelar', class:'btn', action:function(e,ov){ closeDlg(ov); } });
                                       $('body').dialog({
                                           title: aEl ? 'Editar enlace' : 'Insertar enlace',
                                           type: 'html',
                                           content: form,
                                           width: 'auto',
                                           height: 'auto',
                                       ...
                                       setTimeout(function(){ var u=document.getElementById('wqlink-url'); if(u){ u.focus(); u.select(); } }, 50);
                                   }else{
