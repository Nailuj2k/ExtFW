<?php
        ...
        function editable($id=false){
            ...
            if($id)
            $item = Table::getFieldsValues("SELECT item_id,item_parent FROM ".TB_ITEM." WHERE item_url = (SELECT item_name FROM ".TB_PAGES." WHERE item_id='".(int)$id."')");
            else
            $item = Table::getFieldsValues("SELECT item_id,item_parent FROM ".TB_ITEM." WHERE item_url= '".ITEM_NAME."'");
            ...
            if(is_array($ir)&&is_array($ur)){
                ...
                if($parent_id>0) {
                    ...
                    while ($parent_id>0){
                        ...
                        $item = Table::getFieldsValues("SELECT item_id,item_parent FROM ".TB_ITEM." WHERE item_id = '".(int)$parent_id."'");
                        $module_id = $item['item_id']; 
                        $parent_id = $item['item_parent'];
                        if($parent_id>0) { 
                            $parents[] = $parent_id;
