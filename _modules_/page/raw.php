...
// === Anti-loop guard ===
// Pagar a una LNAddress servida por ESTE mismo sitio crea otra factura en el
// mismo store BTCPay, que vuelve a settlear y a disparar este webhook, en bucle
// con -5% de comision por vuelta. Rechazar cualquier destino propio.
$selfHosts = array_filter(array_map('trim', explode(',',
    $cfg_btc['btcpay.self_hosts'] ?? 'noxtr.net')));

$isSelfLnAddress = function($lnAddress) use ($selfHosts) {
    $lnAddress = strtolower(trim((string)$lnAddress));
    $parts = explode('@', $lnAddress);
    if (count($parts) !== 2) return false;
    $domain = $parts[1];
    foreach ($selfHosts as $h) {
        $h = strtolower($h);
        if ($domain === $h || substr($domain, -strlen('.'.$h)) === '.'.$h) {
            return true;
        }
    }
    return false;
...
if($btcpay_webhook){
    ...
    if ($eventType === 'InvoiceSettled') {
        ...
        if ($moduleId == 5 && !$authorId && $amountSats > 0) {
            ...
            if ($isSelfLnAddress($lnAddress)) {
                $writeWebhookLog('webhook_SELF_PAYMENT_BLOCKED',
                    "Anti-loop: destino propio $lnAddress, no se reenvia el zap");
            } else if ($lnAddress) {
        ...
        if ($authorId && $amountSats > 0) {
            ...
            if (!empty($authorLnAddress) && $authorAmountSats > 0 && $isSelfLnAddress($authorLnAddress)) {
                $writeWebhookLog('webhook_SELF_PAYMENT_BLOCKED',
                    "Anti-loop: lightning_address propia $authorLnAddress, se acumula en balance");
            } else if (!empty($authorLnAddress) && $authorAmountSats > 0) {
