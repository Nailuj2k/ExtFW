    ...
    if ( Vars::IsNumeric($item_id) ){
        ...
        if($editable) { 
            ...
            if       ($_ARGS['op'] == 'save_file'){
                ...
                $arr_img_types = ['image/png', 'image/gif', 'image/jpeg', 'image/webp'];
                $arr_allowed_types = array_merge($arr_img_types, ['application/pdf',
                    'application/msword',
                ...
                // Detectar MIME real en servidor, ignorar el tipo enviado por el cliente
                $finfo     = new finfo(FILEINFO_MIME_TYPE);
                $real_type = $finfo->file($_FILES['file']['tmp_name']);

                if (!in_array($real_type, $arr_allowed_types, true)) {
                    $result['error'] = 1;
                    $result['msg']   = 'File type not allowed.';
                    echo json_encode($result);
                    exit;
                }

                $valid_img_ext = in_array($real_type, $arr_img_types, true);
                $ahora = time();
                ...
                @mkdir(SCRIPT_DIR_MEDIA, 0755, true);
                @mkdir($dir_media_module_images, 0755, true);
                @mkdir($dir_media_module_images_id, 0755, true);
                if (move_uploaded_file($_FILES['file']['tmp_name'], $dir_media_module_images_id .'/'. $filename)){
                    ...
                    //img_resize($dir_media_module_images_id , $filename);
                        ...
                        if     ($real_type=='image/png') png2webp( $dir_media_module_images_id.'/'.$filename );
                        else if($real_type=='image/jpeg') jpeg2webp( $dir_media_module_images_id.'/'.$filename );
                    }else if($real_type=='application/pdf'){
                        $provider = '5';
                    }else{
              ...
              //$field_text_name = $_ARGS['lang']?$_FNAME_.'_'.$_ARGS['lang']:$_FNAME_;
                ...
                //      if(!Root()){
                        ...
                        $AllowTag = array('a', 'img', 'br', 'strong', 'b', 'code', 'pre', 'p', 'div', 'em', 'span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'style',
                         'table', 'colgroup', 'thead', 'tbody', 'tfoot', 'tr', 'th', 'td', 'hr', 'li', 'u', 'input','button','textarea','figure','blockquote','figcaption','iframe');
                        //include(SCRIPT_DIR_CLASSES.'/xss.class.php');

                        //FIX: Permitir comentarios html <!-- -->
                ...
                if($cfg['db']['type']  != 'mysql'){
                    ...
                    if($_ARGS['addmenu']==1){
                        ...
                        $sql_menu = "INSERT INTO ".TB_ITEM." ( item_parent,item_caption,item_name,item_url,id_menu,item_active,item_visible,item_order) 
                                                    VALUES ('".$p."','".$title_safe."','". $row_menu[0]['item_name'].'-'.$name."','".$item_url."','".max('0',$row_menu[0]['id_menu'])."','1','1','".max('0',$row_menu[0]['item_order'])."')";
                    ...
                    Table::sqlExec("UPDATE temp_table SET item_id=0,item_name=concat(item_name,'-','".$name."'),item_title='".$title_safe."' WHERE item_id=".$item_id);
                    ...
                    $sql_page = "CREATE TEMPORARY TABLE temp_table AS SELECT * FROM ".TB_PAGES." WHERE item_id=".$item_id."\n"
                              . "UPDATE temp_table SET item_id=0,item_name=concat(item_name,'-','".$name."'),item_title='".$title_safe."' WHERE item_id=".$item_id."\n"
