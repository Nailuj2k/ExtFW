        ...
        */
        ...
        $('body').on('change','#FIELD_TYPE',function(e){
            ...
            let sel=$(this).val()=='select';                       
                if (sel) $("#FIELD_LEN").val("").closest(".control-group").show("fast");
...
<?php } ?>
        ...
        // ── Live del log del rate limiter (pestaña Rate) ─────────────────────────
        // Mismo patrón que el Live de arriba, contra el endpoint 'ratelog'
        // (rate_YYYYMMDD.log, líneas JSON de LOG::rate). Fuera del gate
        // CFG::$vars['log']: LOG::rate escribe siempre.
        const ratelogLines = document.getElementById('ratelog-lines');
        var ratelog_polling = false;
        var ratelogIntervalId = null;
        let ratelogLastPosition = 0;

        if (document.getElementById('start-stop-ratelog-live')) {
            document.getElementById('start-stop-ratelog-live').addEventListener('click', function() {
                ratelog_polling = !ratelog_polling;
                if (ratelog_polling) {
                    this.innerHTML = 'Detener &nbsp; <i class="fa fa-pause"></i>';
                    updateRateLog();
                    ratelogIntervalId = setInterval(updateRateLog, 2000);
                    this.classList.add('btn-danger');
                    this.classList.remove('btn-primary');
                } else {
                    this.innerHTML = 'Reanudar &nbsp; <i class="fa fa-play"></i>';
                    this.classList.remove('btn-danger');
                    this.classList.add('btn-primary');
                    clearInterval(ratelogIntervalId);
                }
            });
        }

        var ratelogPlaceholderShown = false;
        function updateRateLog() {
            fetch('<?=MODULE?>/ajax/ratelog/pos=' + ratelogLastPosition)
                .then(response => response.text())
                .then(text => {
                    let data;
                    try {
                        ...
                        console.error('Error al parsear JSON (ratelog):', e, text);
                        clearInterval(ratelogIntervalId);
                        ratelog_polling = false;
                        return;
                    }
                    if (data.new_content) {
                        if (ratelogPlaceholderShown) { ratelogLines.textContent = ''; ratelogPlaceholderShown = false; }
                        ratelogLines.append(data.new_content);
                        ratelogLastPosition = data.new_position;
                        ratelogLines.scrollTo({ top: ratelogLines.scrollHeight, behavior: 'smooth' });
                    } else if (data.new_position) {
                        ratelogLastPosition = data.new_position;
                    } else if (!ratelogLines.textContent) {
                        // Log de hoy vacío o inexistente: avisar en vez de dejar el visor mudo.
                        // Solo registra bloqueos y casi-bloqueos — sin tráfico excesivo no
                        // escribe nada (y requiere el include del ratelimiter activo en run.php).
                        ratelogLines.textContent = '— Sin eventos hoy. El rate log solo registra bloqueos y casi-bloqueos (<20% de cupo). Usa el botón Test para comprobar que el visor funciona. —';
                        ratelogPlaceholderShown = true;
                    }
                })
                .catch(err => console.error('Error (ratelog):', err));
        }

        // Botón Test: escribe una línea de prueba vía LOG::rate y arranca el polling si no
        // estaba — la línea debe aparecer en el visor en ≤2s si toda la tubería funciona.
        var ratelogTestBtn = document.getElementById('ratelog-test-btn');
        if (ratelogTestBtn) {
            ratelogTestBtn.addEventListener('click', function() {
                fetch('<?=MODULE?>/ajax/ratelog_test')
                    .then(r => r.json())
                    .then(() => {
                        if (!ratelog_polling) document.getElementById('start-stop-ratelog-live').click();
                    })
                    .catch(err => console.error('Error (ratelog_test):', err));
            });
        }

        // Pausar/reanudar el polling al cambiar de pestaña, encadenado con el
        // onActivateTab del Live normal si existe (simpleTabs llama al global).
        var _prevOnActivateTab = typeof onActivateTab === 'function' ? onActivateTab : null;
        window.onActivateTab = function(event, id) {
            if (_prevOnActivateTab) _prevOnActivateTab(event, id);
            if (id === 'tab-cfg-log-rate') {
                if (ratelog_polling) {
                    updateRateLog();
                    clearInterval(ratelogIntervalId);
                    ratelogIntervalId = setInterval(updateRateLog, 2000);
                ...
                clearInterval(ratelogIntervalId);
                ratelog_polling = false;
                var rbtn = document.getElementById('start-stop-ratelog-live');
                if (rbtn && rbtn.classList.contains('btn-danger')) {
                    rbtn.innerHTML = 'Reanudar &nbsp; <i class="fa fa-play"></i>';
