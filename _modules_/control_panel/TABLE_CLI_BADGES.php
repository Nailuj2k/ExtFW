...
class badgesEvents extends defaultTableEvents implements iEvents{ 
  ...
  function OnInsert($owner,&$result,&$post) { 
      ...
      if(!$post['NAME']) {
          ...
          $rows = Table::sqlQueryPrepared('SELECT COUNT(*) AS n FROM CLI_BADGES WHERE NAME = ?', [$post['NAME']]);
          $r = (int)($rows[0]['n'] ?? 0);
          if($r >0) {
              $result['error'] =5;
              $result['msg'] = 'Ya existe una fila con el identificador '.htmlspecialchars($post['NAME']);
          }else{
          }
      }
  ...
  function OnUpdate($owner,&$result,&$post) { 
      ...
      if(!$post['NAME']) {
          ...
          $rows = Table::sqlQueryPrepared('SELECT COUNT(*) AS n FROM CLI_BADGES WHERE NAME = ? AND ID <> ?', [$post['NAME'], (int)$post['ID']]);
          $r = (int)($rows[0]['n'] ?? 0);
          if($r >0) {
              $result['error'] =5;
              $result['msg'] = 'Ya existe una fila con el identificador '.htmlspecialchars($post['NAME']);
  ...
  function OnAfterUpdate($owner,&$result,&$post){
      if($post['fake_input_FILE_NAME']=='') Table::sqlQueryPrepared('UPDATE '.$owner->tablename." SET FILE_NAME='' WHERE ID=?", [(int)$post['ID']]);
  }
