...
<div id="tab-cfg-stats" style="text-align:center;">

<div style="display:none;text-align:right;margin-bottom: -10px;padding: 8px;">
    <!--
    ...
    -->
    <a class="btn btn-success" id="btn-load-stats"> <i class="fa fa-bar-chart"></i> Recargar </a>

</div>

<div id="stats-content" style="width:48%;display:inline-block;"> 
</div>                

<?php
/***************************************
$sql ="SELECT CFG_CALENDAR.datefield AS date,
       IFNULL(COUNT(CLI_ORDERS.ORDER_ID),0) AS orders,
       IFNULL((SELECT SUM(CLI_ORDER_LINES.UD*CLI_ORDER_LINES.PRODUCT_PRICE) FROM CLI_ORDER_LINES WHERE ORDER_ID=CLI_ORDERS.ORDER_ID),0) AS total
FROM CLI_ORDERS RIGHT JOIN CFG_CALENDAR
    ON (DATE(CLI_ORDERS.ORDER_DATE) = CFG_CALENDAR.datefield)
WHERE (CFG_CALENDAR.datefield BETWEEN (SELECT MIN(DATE(ORDER_DATE)) FROM CLI_ORDERS) AND (SELECT MAX(DATE(ORDER_DATE)) FROM CLI_ORDERS))
  AND CLI_ORDERS.ORDER_STATE IN(2,5,7)
  AND CLI_ORDERS.ORDER_DATE LIKE '2023%' 
GROUP BY date";



$sql ="SELECT CFG_CALENDAR.datefield AS date,CLI_ORDERS.ORDER_DATE,
       IFNULL(COUNT(CLI_ORDERS.ORDER_ID),0) AS orders,
       IFNULL((SELECT SUM(CLI_ORDER_LINES.UD*CLI_ORDER_LINES.PRODUCT_PRICE) FROM CLI_ORDER_LINES WHERE ORDER_ID=CLI_ORDERS.ORDER_ID),0) AS total
FROM CLI_ORDERS RIGHT JOIN CFG_CALENDAR
    ON (DATE(CLI_ORDERS.ORDER_DATE) = CFG_CALENDAR.datefield)
WHERE (CFG_CALENDAR.datefield BETWEEN (SELECT MIN(DATE(ORDER_DATE)) FROM CLI_ORDERS) AND (SELECT MAX(DATE(ORDER_DATE)) FROM CLI_ORDERS))
  AND  (CFG_CALENDAR.datefield  LIKE '2023%' ) 
  AND CLI_ORDERS.ORDER_STATE IN(2,5,7)
GROUP BY date";


    $r0 = Table::sqlQuery($sql);
    //print_r( $r0);
    $data = json_encode($r0);          
    //echo $data;
***************************/
...
<style>
/*
#chart-container {	margin: 0rem auto;	max-width: 44rem;	padding: 1rem;}
#chart-container > * + * {	margin-top: 1.5em;}
header > * + *,
article > * + * {	margin-top: 0.25em;}
article svg {	width: 100%;	height: auto;	display: block;}
*/
</style>
<div id="chart-container" style="vertical-align:top;padding-top:30px;display:inline-block;min-width:400px; border:0px solid red;width:50%;height:450px;margin: 0 auto;">

</div>
<?php
$r0 = Table::sqlQuery("SELECT YEAR(ORDER_DATE) AS year, MONTH(ORDER_DATE) AS month, COUNT(*) AS value FROM CLI_ORDERS GROUP BY YEAR(ORDER_DATE), MONTH(ORDER_DATE)");
//print_r( $r0);
$data = str_replace(['"year"','"month"','"value"'],['year','month','value'],json_encode($r0));          
//   echo '<pre>'.$data.'</pre>';
...
const root = select("#chart-container"); //.append("div").attr("id", "root");
...
plotData({
// title: "Female",
// description: "Olivia edged out Emma, even if both choices followed a decreasing trend. The popularity of all names is quite recent, being used at most a thousand times in 1981.",
data: [...dataset.orders].sort(
(a, b) =>
...
.domain(names)
.range(names.map((_, i, { length }) => interpolateRainbow((1 / length) * i)));
...
const wrapper = root //old: article
.append("svg")
...
<?php if(1==2){?>
<!--<pre id="result">  ... </pre>-->
<div id="chart-container" style="display:inline-block;min-width:400px; width:50%;height:450px;margin: 0 auto;"><canvas id="mycanvas"></canvas><button class="btn btn-success btn-small" style="float: left;" id="toggleScale">Escala linear</button></div>
  
<!--<script type="text/javascript" src="<?=SCRIPT_DIR_MODULES?>/stats/Chart.min.js?ver=1.1"></script>-->
<script type="text/javascript">
    window.chartColors = {
        red: 'rgb(255, 99, 132)',
    ...
    function promedio(array){
        let values = array.slice(-3);
        values.sort((a, b) => a - b);
        let p = (values[(values.length - 1) >> 1] + values[values.length >> 1]) / 2;
        console.log(values,p);
        return p;   
    }

    function media_ar(array){
        let values = array.slice(-10);
        let sum = values.reduce((previous, current) => current = parseInt(current) + parseInt(previous));
    ...
    for(var i in rows) {
         a = parseFloat(a) + parseFloat(rows[i].orders);
         b = parseFloat(b) + parseFloat(rows[i].total);
         pedidos.push(rows[i].orders);
         media.push(media_ar(pedidos));
         fechas.push(rows[i].date);
         totales.push(rows[i].total);
         acum.push(a.toFixed(2));
         iacum.push(b.toFixed(2));
     }
     var type = 'linear';
     //  var ctx = $("#mycanvas");
