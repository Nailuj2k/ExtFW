         ...
         $_dirphotos = SCRIPT_DIR_MEDIA.'/avatars/';      //$this->colByName('user_url_avatar')->uploaddir;
               ...
               if (isset($_FILES['croppedImage']) && $_FILES['croppedImage']['error'] === UPLOAD_ERR_OK) {
                  ...
                  //    $result['msg'] = 'No se puede escribir en /media/NEWS/files/11';
                        ...
                        /***********************************
                        include_once( SCRIPT_DIR_LIB.'/facedetector/FaceDetector2.php');
                        $detector = new svay\FaceDetector(SCRIPT_DIR_LIB.'/facedetector/detection.dat');
                        $ok = $detector->faceDetect($filePath);
                        if($ok!==false){
                            //$face_detect->toJpeg();
                            //$face_detect->cropFace();
                            // imagejpeg( $face_detect->cropFace(), $file, 99);

                            $result['msg'] = 'Foto válida';  //: <img style="height:20px;" src="'.$filePath.'?hash='.time().'">';  //ID: '.$params['id'].'<br /> SQL: SELECT * FROM '.$prefix.'_user WHERE user_id='.$params['id'].'<br />
                            $result['image'] = $filePath;
                            //$result['msg'] = '<img src="/'.$filePath.'?hash='.time().'">';  //ID: '.$params['id'].'<br /> SQL: SELECT * FROM '.$prefix.'_user WHERE user_id='.$params['id'].'<br />
                            $result['img']=$filePath.'?hash='.time();
                            // Only update user_url_avatar for avatar uploads, not banners
                           ...
                           unlink($filePath);
                           $result['error']=1;
                           $result['msg'] =  'No se encontrado una cara válida en la foto';
                            //$result['msg'] =  'No se encontrado una cara válida en la foto<div style="font-size:0.5em;font-weight:normal;">El fondo de la foto debe ser claro y la cabeza debe ocupar al menos 2 terceras partes de la foto, y estar mas o menos centrada</div>';
                        }
                        ***************/
