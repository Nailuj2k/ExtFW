...
$tabla->slider        = $tabla->slider       ? $tabla->slider           : false;
...
//Vars::debug_var("SELECT CATEGORIE_ID AS ID, NAME FROM CLI_CATEGORIES WHERE ACTIVE=1 AND CATEGORIE_ID IN (SELECT DISTINCT(CATEGORIE_ID) FROM ".$tabla->tablename." WHERE ".$tabla->fk." = ".$_SESSION['PAGE_FILES_ID_PARENT'].")    ORDER BY CAT_ORDER");
    if(isset($_SESSION['PAGE_FILES_ID_PARENT'])&&$_SESSION['PAGE_FILES_ID_PARENT']>0){
        $_pf_parent = (int)$_SESSION['PAGE_FILES_ID_PARENT'];
        $categorie->values          =  $tabla->toarray('categories',         "SELECT CATEGORIE_ID AS ID, NAME FROM ".$tabla->tb_categories." WHERE ACTIVE=1 AND ID_PARENT  = ".$_pf_parent." ORDER BY CAT_ORDER",true);
        $categorie->values_all      =  $tabla->toarray('categories_all',     "SELECT CATEGORIE_ID AS ID, NAME FROM ".$tabla->tb_categories." WHERE ACTIVE=1 AND ID_PARENT  = ".$_pf_parent." ORDER BY CAT_ORDER",true);
//        $categorie->values_visibles =  $tabla->toarray('categories_visible', "SELECT CATEGORIE_ID AS ID, NAME FROM ".$tabla->tb_categories." WHERE ACTIVE=1 AND ID_PARENT  = ".$_pf_parent.") AND CATEGORIE_ID IN (SELECT DISTINCT(ID_CATEGORIE) FROM ".$tabla->tablename." WHERE ".$tabla->fk." = ".$_pf_parent.") AND VISIBLE=1 ORDER BY CAT_ORDER",true);
        $categorie->values_visibles =  $tabla->toarray('categories_visible', "SELECT CATEGORIE_ID AS ID, NAME FROM ".$tabla->tb_categories." WHERE ACTIVE=1  AND CATEGORIE_ID IN (SELECT DISTINCT(ID_CATEGORIE) FROM ".$tabla->tablename." WHERE ".$tabla->fk." = ".$_pf_parent.") AND VISIBLE=1 ORDER BY CAT_ORDER",true);
    }elseif($parent){
...
$slider->fieldname = 'SLIDER';
$slider->label     = 'Slider';   
$slider->textafter = 'Usar ésta imagen para slider.';
$slider->editable  = Administrador();  
}
...
if($tabla->profile=='default'){
    ...
    if($tabla->slider) $tabla->addCol($slider);
...
if($tabla->table_tags)   {   
    $_tag_safe = addslashes($_SESSION[$tabla->tablename]['tag'] ?? '');
    $tabla->where = $_tag_safe ? 'ID IN (SELECT FILE_ID FROM '.$tabla->tablename.'_TAGS WHERE TAG_ID = (SELECT ID FROM '.$tabla->table_tags.' WHERE NAME=\''.$_tag_safe.'\'))' : '';
}
...
class filesEvents extends defaultTableEventsTags implements iEvents{ 
        ...
        if      ($row['ID_PROVIDER']=='1') {
            ...
            */
            ...
            if ($type=='txt'){
                ...
                $target = 'target="new"';
                $row['NAME'] = '<a target="new" '.$href.' item="'.$row['ID'].'" download="'.$name.'">'.$row['NAME'].'</a>';
            }
    ...
    function OnInsert($owner,&$result,&$post) { 
        ...
        $type = '8';
        $log_sql = 'INSERT INTO '.TB_LOG.' (TYPE,ID_USER,EMAIL,SUBJECT,MESSAGE) VALUES(\''.$type.'\','.(int)$_SESSION['userid'].',\''.addslashes($_SESSION['user_email']).'\',\''.MODULE.'_files - insert - inline\',\''.Str::escape(print_r($_FILES,true)."\n".print_r($post,true)).'\')';
