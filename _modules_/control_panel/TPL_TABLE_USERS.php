...
$activo->label     = 'Activo';   
$activo->editable  = !$tabla->profile && (Operador()||Administrador());
$activo->readonly  = !$activo->editable ;   
...
$avatar->image_width  = '260px';
$avatar->image_height = '320px';
$avatar->ratio = 'dni';
...
$avatar->value='dummy.jpg';

$avatar->inline_edit = false;
$avatar->accepted_extensions =  $cfg['accepted_img_extensions'] ; //array( '.jpg');  
...
if(CFG::$vars['login']['nostr']['enabled']){
  ...
  $nostr_banner->action_if_exists = 'replace';
    ...
    $NOSTR_USER = new Field();
    $NOSTR_USER->type      = 'varchar';
    $NOSTR_USER->len       = 20;
    $NOSTR_USER->fieldname = 'NOSTR_USER';
    $NOSTR_USER->label     = 'Usuario Nostr';   
    $NOSTR_USER->editable   = true;
    $NOSTR_USER->searchable = true;
    $NOSTR_USER->hide=true;
    $NOSTR_USER->fieldset = 'nostr';
...
$verify->editable  = Administrador() || Operador();
...
$api_key->editable  = Administrador() || Operador();
...
$device_link_token->editable  = Administrador();
...
$device_link_expires->editable  = Administrador();
...
if($tabla->tablename==TB_USER){
  //if (!$tabla->profile && Administrador())
     $tabla->addCol($activo);
...
$extra_fields = $tabla->query2array("SELECT * FROM CFG_EXTRA_FIELDS WHERE T4BLE_NAME='".$tabla->tablename."' AND ACTIVE = '1' ORDER BY FIELDSET,FIELD_ORDER");  
...
//$print_label = true;
$tabla->extra_fields_parent_selects = [];
...
foreach ($extra_fields as $k => $v) {
   ...
   //   ${$column}->accepted_doc_extensions   = explode(',',$v['EXTENSIONS']);
    ...
    if($v['FIELD_TYPE']=='select'){
        ${$column}->values  = $tabla->toarray( 'v_'.$column ,  'SELECT '.$v['LOOKUP_FIELD_KEY'].' AS ID, '.$v['LOOKUP_FIELD_NAME'].' AS NAME FROM '.$v['LOOKUP_FIELD_TABLE'].' ORDER BY '.$v['LOOKUP_FIELD_NAME'],true);
        ...
        */
        ...
        if($v['CHILD_FIELD_NAME'] && $v['CHILD_SQL'] && $v['CHILD_AJAX_URL']){

            $tabla->extra_fields_parent_selects[]= $column;

            $_sql='select_'.$v['CHILD_FIELD_NAME']; //Str::sanitizeName($v['CHILD_SQL']);
            $tabla->sqls[$_sql]=$v['CHILD_SQL']; 
            ${$column}->child_fieldname  = $v['CHILD_FIELD_NAME'];
            ${$column}->child_ajax_url   = $v['CHILD_AJAX_URL'];
            ${$column}->child_source_sql = $_sql;

            //"SELECT ".$v['CHILD_FIELD_KEY']." AS ID, ".$v['CHILD_FIELD_NAME']." AS NAME FROM ".$v['CHILD_FILED_TABLE']." WHERE ".$v['LOOKUP_FIELD_KEY']."='[PARENT_VALUE]'";
        }
...
if(Administrador()||Operador()){  
  ...
  $tabla->perms['add']    = (Administrador()||Operador()) && $tabla->tablename==TB_USER;
  ...
  $tabla->perms['print']  = Administrador()||Operador();  
...
class userEvents extends defaultTableEvents implements iEvents{ 
  ...
  function OnShow($owner){
    ...
    function list_messages($owner,$args){
        $result = array();
        $result['error']=0;
        $rows_last_messages = Table::sqlQueryPrepared('SELECT ID,ID_USER,TYPE,EMAIL,SUBJECT,MESSAGE,EVENT_DATE FROM LOG_EVENTS WHERE TYPE IN (1,2) AND ID_USER = ? ORDER BY EVENT_DATE DESC LIMIT 10', [(int)$args['id']]);
  ...
  function validate($owner,$args){
      ...
      */
      ...
      $allowed_fields = ['username', 'user_email', 'user_card_id'];
      if (!in_array($args['field'], $allowed_fields, true)) {
          $result['error'] = 1; $result['msg'] = 'Invalid field'; echo json_encode($result); return;
      }
      $rows = Table::sqlQueryPrepared("SELECT count(*) AS n FROM ".TB_USER." WHERE ".$args['field']." = ?", [$args['value']]);
      $result['field'] = $rows[0]['n'] ?? 0;
      $result['sql']=$sql;
      $result['error']=0;
      echo json_encode($result);   
  }
  ...
  function OnAfterShowForm($owner,&$form,$id){
    ...
    ****/
    ...
    if($owner->extra_fields_parent_selects){
      echo '<script>';
      foreach($owner->extra_fields_parent_selects as $k=>$v){
         echo "document.getElementById('$v').dispatchEvent(new Event('change'));\n";
      }
      echo '</script>';
    }
  ...
  function OnUpdate($owner, &$result, &$post){ 
     ...
     if ( Cliente() && !(Operador()||Administrador())  ){
  ...
  function OnDrawRow($owner,&$row,&$class){
    ...
    if ( Usuario() && !(Operador()||Administrador())  ){
      $class='';
  ...
  ****/
  function OnBeforeUpdate($owner,$id){  
    global $_ACL;
    if ( Usuario() && !(Operador()||Administrador()) && !$_ACL->hasPermission('pedidos_admin') ){
  ...
  function NNN_OnDrawCell($owner,&$row,&$col,&$cell){
...
#customer td, #customers th {border: 1px solid #ddd;padding: 8px;}
...
#customer th {padding-top: 2px;padding-bottom:2px;text-align:left;background-color:#b9ad7d;color:white;}
</style>';

$tabla->format_item['body'] = $style.'<table class="zebra table-values" id="customer">'
                         //   . '<tr><th colspan="3"></th></tr>'
                            . '<tr><td>Id</td><td>[user_id]</td>'
                            . '<td rowspan="10" style="background:white;border-bottom:none;"><img class="editable-image" style="width:118px;" src="[user_url_avatar]"></td>' 
                            . '</tr>'
                            . '<tr><td>'.t('USERNAME').'</td><td>[username]</td></tr>'
                         ...
                         // . '<tr><td>Token</td><td>'.$_SESSION['token'].'</td></tr>'
                            ...
                            . '<tr><td>'.t('API_KEY').'</td><td>[api_key]</td></tr>'
