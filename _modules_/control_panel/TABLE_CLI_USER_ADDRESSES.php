...
if(!Administrador() || MODULE=='login' || $_SESSION['userid']) {
     ...
     $card_id->default_value = $tabla::getFieldValue('SELECT user_card_id FROM '.TB_USER.' WHERE user_id = '.(int)$_SESSION['userid']);
...
if(!Administrador() || MODULE=='login') {
   $tabla->where = 'id_user='.(int)$_SESSION['userid'];
...
class myTableMysql extends TableMysql{
    ...
    public function getAddressAsString($params){
        $result = array();
        $result['error']=0;
        $_uid = (int)$_SESSION['userid'];
        $id_county = self::getFieldValue('SELECT ID_COUNTY FROM '.$this->tablename." WHERE id_user = ".$_uid." AND ID_TYPE='".$params['type']."' AND ".($params['id'] ? " USER_ADDRESS_ID=".$params['id'] : " BYDEFAULT='1' "));
        ...
        if(CFG::$vars['users']['field']['county']){
            if($id_county){ 
                $sql = "SELECT ca.*,CONCAT(d.localidad_name,'<br />',ca.ZIP,' ',c.municipio_name,'<br />',b.provincia_name,'<br />',a.pais_name) AS SHIPPING_ADDRESS "   // d.localidad_name,'<br />',d.localidad_cp,'<br />',
                     ...
                     ." AND ca.id_user = ".$_uid." AND ca.ID_TYPE='".$params['type']."'"
...
class addressesEvents extends defaultTableEvents implements iEvents{ 
    ...
    public function OnBeforeShow($owner){
        $_uid = (int)$_SESSION['userid'];
        $sql ='SELECT * FROM CLI_USER_ADDRESSES WHERE id_user='.$_uid;
        ...
        if($xx){
             ...
             $sql1 = "INSERT INTO CLI_USER_ADDRESSES (id_user,NAME,CARD_ID,PHONE,EMAIL,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP,ID_TYPE,BYDEFAULT,ACTIVE ) "
                    ."SELECT CUSTOMER_ID,NAME,CARD_ID,PHONE,EMAIL,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP, '1','1','1' FROM CLI_ORDERS WHERE CUSTOMER_ID=".$_uid;
             Table::sqlExec($sql1);
             $sql2 = "INSERT INTO CLI_USER_ADDRESSES (id_user,NAME,CARD_ID,PHONE,EMAIL,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP,ID_TYPE,BYDEFAULT,ACTIVE ) "
                    ."SELECT CUSTOMER_ID,INVOICE_NAME,INVOICE_CARD_ID,INVOICE_PHONE,INVOICE_EMAIL,INVOICE_ID_COUNTRY,INVOICE_ID_STATE,INVOICE_ID_CITY,INVOICE_ID_COUNTY,INVOICE_ADDRESS,INVOICE_ZIP, '2','1','1' FROM CLI_ORDERS WHERE CUSTOMER_ID=".$_uid;
             Table::sqlExec($sql2);
        }
    }
  ...
  function OnUpdateOrInsert($owner,&$result,&$post){
        ...
        if($post['BYDEFAULT']==1){
           $owner->sql_exec("UPDATE ".$owner->tablename." SET BYDEFAULT=0 WHERE ID_TYPE={$id_type} AND id_user={$uid}");
        }else{
            $r = $owner->recordCount("SELECT count(*) FROM ".$owner->tablename." WHERE ID_TYPE = {$id_type} AND BYDEFAULT = 1 AND id_user={$uid}");
  ...
  function OnAfterUpdate($owner,&$result,&$post){
     ...
     /**
      if($post['ID_TYPE']==2){ 
          $r0 = $owner->recordCount('SELECT count(*) FROM '.$owner->tablename." WHERE ID_TYPE = '1' AND id_user={$_uid}");
          $sql ='INSERT INTO '.$owner->tablename." (id_user,NAME,PHONE,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP,CARD_ID,EMAIL,BYDEFAULT,ID_TYPE) "
                                        ."SELECT '{$_uid}',NAME,PHONE,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP,CARD_ID,EMAIL,'1','1' FROM {$owner->tablename} WHERE ID_TYPE='2' AND USER_ADDRESS_ID = {$post['USER_ADDRESS_ID']}";
          //$result['msg']=$sql;
          if ($r0<1) $owner->sql_query($sql);
      }else  if($post['ID_TYPE']==1){ 
          $r0 = $owner->recordCount('SELECT count(*) FROM '.$owner->tablename." WHERE ID_TYPE = '2' AND id_user={$_uid}");
          $sql ='INSERT INTO '.$owner->tablename." (id_user,NAME,PHONE,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP,CARD_ID,EMAIL,BYDEFAULT,ID_TYPE) "
                                        ."SELECT '{$_uid}',NAME,PHONE,ID_COUNTRY,ID_STATE,ID_CITY,ID_COUNTY,ADDRESS,ZIP,CARD_ID,EMAIL,'1','2' FROM {$owner->tablename} WHERE ID_TYPE='1' AND USER_ADDRESS_ID = {$post['USER_ADDRESS_ID']}";
          //$result['msg']=$sql;
          if ($r0<1) $owner->sql_query($sql);
      }else{
       ****/
          //$result['msg']=print_r($post,true);
           $r1 = $owner->recordCount('SELECT count(*) FROM '.$owner->tablename." WHERE ID_TYPE = '1' AND BYDEFAULT = 1 AND id_user={$_uid}");
           $r2 = $owner->recordCount('SELECT count(*) FROM '.$owner->tablename." WHERE ID_TYPE = '2' AND BYDEFAULT = 1 AND id_user={$_uid}");
           if ($r1<1) $owner->sql_exec('UPDATE '.$owner->tablename." SET BYDEFAULT=1 WHERE ID_TYPE='1' AND id_user={$_uid}");
           if ($r2<1) $owner->sql_exec('UPDATE '.$owner->tablename." SET BYDEFAULT=1 WHERE ID_TYPE='2' AND id_user={$_uid}");
     // }
        ...
        //}else{
            if($_SESSION['userid'] && $post['CARD_ID'] && $post['id_user'] && $post['id_user']>0 && $post['id_user']==$_SESSION['userid']){
                  Table::sqlQueryPrepared("UPDATE ".$owner->tablename." SET CARD_ID=? WHERE id_user=? AND IFNULL(CARD_ID,'')=''", [$post['CARD_ID'], (int)$post['id_user']]);
            }
        //}
