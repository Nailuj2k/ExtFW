<?php
    ...
    if       ( Root()          && $_ARGS['op']=='save_cfg'        ){
        ...
        $configuration_content='<'.'?php
            ...
            '.($_ARGS['master_password'] ?'define(\'MASTER_PASSWORD\'    , \''.addslashes($_ARGS['master_password']).'\');':'').'
        ...
        if (copy($filename, $bak_dir.'/'.$filename)){
            if($f = fopen($filename,'w+')) {
                if(@fwrite($f,$configuration_content)) { //serialize($data))) {
                    ...
                    // activate langs in CFG_LANGS if code is in $_ARGS['langs'] and deactivate if not in $_ARGS['langs']
                    $_langs_cc = "'".str_replace(",","','", $_ARGS['langs'])."'";  // 'en','es','fr'
                    $_sql_lans_cc_1 = "UPDATE CFG_LANG SET lang_active = 1 WHERE lang_cc IN ($_langs_cc)";
                    $_sql_lans_cc_0 = "UPDATE CFG_LANG SET lang_active = 0 WHERE lang_cc NOT IN ($_langs_cc)";
                    Table::sqlQuery($_sql_lans_cc_1);
                    Table::sqlQuery($_sql_lans_cc_0);

                    $result['msg']   = $filename.' actualizado';      
               } else $result['msg']   = "Error::No se puede escribir en el archivo ".$filename;
            } else $result['msg']   = "Error::No se puede abrir el archivo ".$filename;
        } else $result['msg']   = "Error::No se pudo guardar copia de seguridad del archivo ".$filename;
    ...
    }else if ( Administrador() && $_ARGS[2]=='ratelog_test'       ){
        // Escribe una línea de prueba en el rate log para verificar la tubería completa
        // (LOG::rate → rate_YYYYMMDD.log → endpoint ratelog → visor Live). El log real solo
        // registra bloqueos/casi-bloqueos, así que sin tráfico excesivo queda vacío y no
        // habría forma de saber si el visor funciona.
        header('Content-Type: application/json');
        LOG::rate('test', [
            'key'       => 'test',
    ...
    }else if ( Administrador() && $_ARGS[2]=='ratelog'            ){
        // Tail del log del rate limiter (LOG::rate() → rate_YYYYMMDD.log, una línea JSON
        // por evento). Mismo protocolo pos/new_content que el endpoint 'log' de arriba.
        // No depende de CFG::$vars['log'] (LOG::rate escribe siempre).
        $logFile = SCRIPT_DIR_LOG.'/rate_'.date('Ymd').'.log';
        ...
        if ($lastPos <= 1) {
            echo json_encode([
                'new_content' => file_get_contents($logFile),
                'new_position' => $currentSize
            ]);
        } else if ($currentSize > $lastPos) {
            $handle = fopen($logFile, 'r');
        ...
        $result['msg']='Deprecated option: write-cc-file';
        echo json_encode($result);
        ...
        if ($_ARGS[3]=='vendor'){
            ...
            if(!preg_match('/^[a-zA-Z0-9_-]+$/', $_ARGS[4] ?? '')){

                $result['error']=1;
                $result['msg']='Invalid name';

            }else if(in_array($_ARGS[4],['control_panel',/*'edit','marketplace','pads','404','debug','_template_','default',*/'acl'])){
          ...
          //addToZip($hzip,SCRIPT_DIR_MODULES.'/'.$_ARGS[4]);
            ...
            addToZip($hzip,SCRIPT_DIR_I18N.'/en.php',false,$compressed);
            addToZip($hzip,SCRIPT_DIR_I18N.'/es.php',false,$compressed);
            addToZip($hzip,SCRIPT_DIR_I18N.'/fr.php',false,$compressed);
            addToZip($hzip,SCRIPT_DIR_I18N.'/pt.php',false,$compressed);
            addToZip($hzip,SCRIPT_DIR_I18N.'/it.php',false,$compressed);
          ...
          //addToZip($hzip,SCRIPT_DIR_JS.'/html2pdf.bundle.min.js');
            ...
            addToZip($hzip,SCRIPT_DIR_JS.'/bum');
            addToZip($hzip,SCRIPT_DIR_JS.'/sse');
          ...
          //addToZip($hzip,SCRIPT_DIR_LIB.'/html5-qrcode/html5-qrcode.min.js');
            ...
            addToZip($hzip,SCRIPT_DIR_LIB.'/aadsm',false,$compressed);
         ...
         // addToZip($hzip,SCRIPT_DIR_JS.'/ohSnap');
          ...
          //addToZip($hzip,SCRIPT_DIR_MODULES.'/drawing',false,$compressed);
            ...
            addToZip($hzip,SCRIPT_DIR_MODULES.'/install/create_tables_i18n.php');
        ...
        ********************************************************************************************************/ 
        ...
        $result['msg']='Deprecated option: set-mode-auto.';
        echo json_encode($result);
      ...
      //$filename = time() . '_' . Str::sanitizeName($_FILES['file']['name'],true);
       ...
       // $group = posix_getgrgid(filegroup($_SERVER['DOCUMENT_ROOT'].'/index.php'))['name'];
        ...
        LOG::security('ssh_exec', ['cmd' => $cmd, 'user_id' => (int)$_SESSION['userid']]);
