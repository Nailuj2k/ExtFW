    ...
    if(Administrador()){
        ...
        if($_ARGS[2]=='banners'){
         ...
         // $sql = 'SELECT '.implode(',',$fields).' FROM '.$tablename.' WHERE  TIME LIKE \'2022%\' AND  ID_BANNER IN (1,2)';
            $date_from = preg_match('/^\d{4}-\d{2}-\d{2}$/', $_ARGS[3]??'') ? $_ARGS[3] : date('Y-m-d');
            $date_to   = preg_match('/^\d{4}-\d{2}-\d{2}$/', $_ARGS[4]??'') ? $_ARGS[4] : date('Y-m-d');
            $sql = 'SELECT '.implode(',',$fields).' FROM '.$tablename.' WHERE TIME >= \''.$date_from.' 00:00:00\' AND TIME <= \''.$date_to.' 23:59:59\' AND ID_BANNER IN (1,2)';
