...
<form class="login-form form-horizontal form no-ajax" method="post" name="registerform" id="registerform"  action="login" 
    ...
    // El registro con Nostr vive en /login (form_login_login.php): su flujo cubre login Y
    // registro (nostr_verify crea la cuenta si la pubkey es nueva). Aquí solo se informa y
    // se enlaza. Ojo: si algún día se exige invitación, habrá que volver a pasar el código.
    if (CFG::$vars['login']['nostr']['enabled']){
        ?>
        <div id="nostr-register-info" style="padding:16px 18px;margin-bottom:20px;border-radius:var(--radius, 10px);border:2px solid #8B5CF6;background:color-mix(in srgb, #8B5CF6 7%, var(--card, #fff));">
            <strong style="display:block;font-size:1.05em;margin-bottom:4px;">⚡ <?=t('NOSTR_REGISTER_PROMO_TITLE','Regístrate con Nostr','Register with Nostr')?></strong>
            ...
            <a class="btn btn-block btn-nostr" href="/login" style="background:#8B5CF6;color:#fff;border-radius:var(--button-border-radius);">
                ...
                <?=t('NOSTR_GO_TO_LOGIN','Entrar / Registrarse con Nostr','Log in / Register with Nostr')?>
            </a>
        </div>
        <?php
    }
