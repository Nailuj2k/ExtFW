            ...
            <?php 
             ...
             <?php 
                ...
                echo sprintf(TAB_TAB_BEGIN,'tab_test');
                    ...
                    ?>
                    <div>[notificaciones]: <span id="notif">No</span></div>
                    <button id="btn-test-notif">Probar notificaciones</button>
                    <div id="notif-help" style="display:none;max-width:500px;margin-top:8px;padding:10px;background:#fff3cd;border:1px solid #ffc107;border-radius:6px;font-size:13px;">
                        <b>Notificaciones bloqueadas.</b> En macOS hacen falta dos cosas:
                        <ol style="margin:5px 0 0 18px;padding:0;">
                            <li>Ajustes del Sistema &gt; Notificaciones &gt; Microsoft Edge: activadas.</li>
                            <li>Icono junto a la URL &gt; Configuraci&oacute;n del sitio &gt; Notificaciones: <b>Permitir</b> (con "Preguntar" no funciona).</li>
                        </ol>
                    </div>
                    <script type="text/javascript">

                        function dw(t) {
                            document.getElementById('notif').innerHTML = t?'Si':'No';
                        }

                        var isMac = /Mac/.test(navigator.platform);

                        function notifHelp(result) {
                            document.getElementById('notif-help').style.display =
                                (isMac && result != 'granted') ? 'block' : 'none';
                        }
                        ...
                        document.getElementById('btn-test-notif').addEventListener('click', function() {
                            Notification.requestPermission().then(function(result) {
                                ...
                                notifHelp(result);
                                if (result == 'granted') {
                                    new Notification('Tienes un mensaje', { icon: '/media/images/logo.png', body: 'texto' });
                                }
                            });
                        });

                        dw(Notification.permission == 'granted');
                    </script>
                    <?php 
                        ...
                        <?php 
                            ...
                            if( $u && $p ){
                                ...
                                /*    
                                    ...
                                    echo '<br>valid: '.($_SESSION['valid_user']?'1':'0');
                                    echo '<br>save: '.($_SESSION['save']?'1':'0');
                                    echo '<br>user score: '.$_SESSION['user_score'];
                                */    
                                //}
                            }                        
