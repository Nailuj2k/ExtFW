<?php 
            ...
            <?php if ( CFG::$vars['login']['nostr']['enabled']){ ?>
                ...
                async function startNip46Login() {
                    ...
                    try {
                        ...
                        // Keypair de cliente persistente (misma clave que usa el módulo noxtr):
                        // el signer indexa las apps conectadas por client pubkey, así que si
                        // rotara en cada login acumularía "apps conectadas" duplicadas.
                        let clientPriv = null;
                        try {
                            const storedClient = JSON.parse(localStorage.getItem('noxtr_nip46_client') || 'null');
                            if (storedClient && /^[0-9a-f]{64}$/i.test(storedClient.privkey || '')) clientPriv = storedClient.privkey.toLowerCase();
                        } catch(e) {}
                        if (!clientPriv) {
                            clientPriv = bytesToHex(crypto.getRandomValues(new Uint8Array(32)));
                            try { localStorage.setItem('noxtr_nip46_client', JSON.stringify({ privkey: clientPriv })); } catch(e) {}
                        }
                            ...
                            if (!connected && (msg.result === secret || msg.result === 'ack')) {
                                ...
                                try {
                                    ...
                                    if (vData.success) {
                                        ...
                                        // Guardar el estado del canal NIP-46 para que noxtr lo restaure
                                        // (Nip46.restore() lee 'noxtr_nip46') y pueda firmar via signer
                                        // sin re-escanear el QR. Limpiar también los flags de sesión que
                                        // harían que noxtr descartara este estado recién guardado.
                                        try {
                                            localStorage.setItem('noxtr_nip46', JSON.stringify({
                                                clientPrivkey: clientPriv, clientPubkey: clientPub,
                                                signerPubkey: signerPubkey, userPubkey: userPubkey
                                            }));
                    ...
                    $("body").dialog({
                        ...
                        content: `
                            <div style="padding:16px 20px;">
                                <p style="margin:0 0 14px;color:#666;font-size:0.9em;">
                                    No se ha detectado una extensión Nostr (NIP-07). Métodos disponibles:
                                </p>

                                <div id="nostr-opt-nip46" class="nostr-login-opt" style="border:1px solid #d5c8f5;border-radius:8px;padding:12px 14px;margin-bottom:10px;cursor:pointer;text-align:left;">
                                    <strong style="font-size:0.95em;display:flex;align-items:center;gap:7px;">
                                        <img src="https://signer.noxtr.net/logo.svg" alt="" style="height:20px;width:20px;flex-shrink:0;" onerror="this.style.display='none'">
                                        Firmador remoto (NIP-46) &mdash; signer.noxtr.net
                                    </strong>
                                    <p style="margin:4px 0 0;font-size:0.82em;color:#888;">
                                        Abre <span style="font-family:monospace;">https://signer.noxtr.net</span> en tu móvil y escanea el QR que se mostrará aquí.
                                        La clave privada se genera y permanece en el firmador: este navegador nunca la ve,
                                        y cada firma se aprueba desde el móvil.
                                        También sirve con cualquier otro firmador NIP-46 (Amber, nsec.app...).
                                    </p>
                                </div>

                                <div id="nostr-opt-new" class="nostr-login-opt" style="border:1px solid #d5c8f5;border-radius:8px;padding:12px 14px;margin-bottom:10px;cursor:pointer;text-align:left;">
                                    <strong style="font-size:0.95em;"><i class="fa fa-magic"></i> Crear identidad nueva</strong>
                                    <p style="margin:4px 0 0;font-size:0.82em;color:#888;">
                                        Se genera una identidad Nostr en segundos.
                                        Las claves se guardan en tu navegador y son tuyas para siempre:
                                        podrás usarlas en cualquier app Nostr.
                                    </p>
                                </div>

                                <div id="nostr-opt-ext" class="nostr-login-opt" style="border:1px solid #d5c8f5;border-radius:8px;padding:12px 14px;cursor:pointer;text-align:left;">
                                    <strong style="font-size:0.95em;"><i class="fa fa-puzzle-piece"></i> Extensión de navegador (NIP-07)</strong>
                                    <p style="margin:4px 0 0;font-size:0.82em;color:#888;">
                                        Instala una extensión como nos2x y firma desde este navegador.
                                        Tras instalarla, recarga esta página.
                                    </p>
                                </div>
                            </div>
                        `,
                        onLoad: function(dialog) {
                            ...
                            var wire = function(id, fn) {
                                ...
                                if (el) el.onclick = function() { dialog.close(); setTimeout(fn, 300); };
                            };
                            wire('#nostr-opt-nip46', function() {
                                // Mismo flujo que el botón NostrConnect de la pantalla de login (QR NIP-46)
                                var b = document.getElementById('btn-nostr-connect'); if (b) b.click();
                            });
                            wire('#nostr-opt-new', function() { createNostrIdentity(); });
                            wire('#nostr-opt-ext', function() {
    ...
    // v2 (antes v1): script.js (noxtr) también abre esta misma BD compartida, y su versión antigua
    // de loadStoredKeys() abortaba la transacción de upgrade en vez de crear el object store 'keys'
    // — si esa función llegaba a ser la primera en abrir la BD en un navegador (p.ej. el usuario
    // entra directo a noxtr.net sin pasar antes por /login), la dejaba creada en v1 SIN el store,
    // y como onupgradeneeded solo se dispara una vez por versión, quedaba inservible para siempre
    // (modo solo-lectura persistente, error nativo "object store not found" al intentar usarla).
    // Subir la versión aquí Y en script.js (mismo valor) fuerza que onupgradeneeded se dispare de
    // nuevo para todo el mundo — incluidos los navegadores ya afectados — y se autorepare solo.
    const NOSTR_DB_VERSION = 2;
