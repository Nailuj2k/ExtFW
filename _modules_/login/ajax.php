...
// ============================================================================
    ...
    if ($op == 'nostr_challenge') {
        ...
        // Auto-login desde noxtr (firmador ya conectado, sin sesion web): NO crear
        // usuario nuevo. Solo iniciar sesion si la pubkey ya esta vinculada a una cuenta.
        $existingOnly = !empty($_ARGS['existing_only'] ?? $_POST['existing_only'] ?? '');
        if ($existingOnly && !$existingNostrUser) {
            http_response_code(401);
            echo json_encode(['success' => false, 'error' => 'no_account', 'msg' => 'Esta identidad Nostr no tiene cuenta en el sitio']);
            exit;
        }
        
        // La pubkey firmada no está vinculada a ningún usuario, pero el cliente indica a qué
        // cuenta pertenece (p.ej. backup de noxtr con data.username). Si esa cuenta existe,
        // NO crear un usuario nuevo: avisar de que debe entrar con otro método y vincular
        // la identidad desde dentro (perfil o "Importar" en noxtr).
        if (!$existingNostrUser && $customUsername !== '') {
            $claimRows = NostrAuth::sqlQueryPrepared(
                "SELECT user_id, username, nostr_pubkey FROM " . TB_USER . " WHERE (user_email = ? OR username = ?) LIMIT 1",
                [$customUsername, $customUsername]
            );
            $claimUser = $claimRows[0] ?? null;
            if ($claimUser) {
                $msg = empty($claimUser['nostr_pubkey'])
                    ? 'La cuenta "' . $claimUser['username'] . '" no tiene ninguna identidad Nostr vinculada. Inicia sesión con otro método (contraseña, passwordless o enlace por email) y vincula tu identidad desde tu perfil o con "Importar" en noxtr.'
                    : 'Este backup contiene una identidad Nostr distinta a la vinculada a "' . $claimUser['username'] . '". Inicia sesión con otro método y actualiza la identidad desde tu perfil o con "Importar" en noxtr.';
                http_response_code(401);
                echo json_encode([
