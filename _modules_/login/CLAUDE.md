...
**Nostr:** `nostr_challenge`, `nostr_challenge_for_pubkey`, `nostr_challenge_for_user`, `nostr_verify`, `nostr_link`, `nostr_link_challenge`, `nostr_unlink`

### nostr_verify — username del backup, sin crear usuario nuevo (2026-06-10)

`nostr_verify` acepta un `username` opcional. El login real sigue siendo **solo por firma de la pubkey** (`findUserByPubkey`); el username NO autentica nada. Su único uso: si la pubkey firmada NO está vinculada a ningún usuario pero `username` corresponde a una cuenta existente (username o email), **no se crea un usuario nuevo** — responde 401 `{error:'account_not_linked', username, msg}` con un aviso de que debe iniciar sesión con otro método (contraseña/passwordless/email) y vincular la identidad desde su perfil o con "Importar" en noxtr. El `msg` distingue cuenta sin identidad vinculada vs identidad distinta a la del backup.

Origen: el botón "backup" del login nsec de noxtr envía `data.username` del JSON de backup (`autoLoginNostr(username)` en `noxtr/script.js`, que muestra el `msg` del servidor en un alert). Evita que restaurar un backup tras una migración registre un usuario `n_xxxxxxxx` nuevo en vez de entrar con la cuenta original. Efecto colateral en el registro Nostr: si el username elegido ya existe, se devuelve `account_not_linked` en lugar de crear un username duplicado. `NostrAuth::findUserByPubkey()` ordena por `user_id ASC` (ante pubkeys duplicadas gana la cuenta más antigua).
...
- **Account lockout**: tras N intentos fallidos de login para un mismo email/usuario, bloquear la cuenta durante X minutos. Implementar en `Login::login()` — registrar intentos fallidos en DB (columna `login_attempts` + `login_locked_until` en `CLI_USER`, o tabla aparte). Desbloqueo automatico por tiempo o por email de recuperacion.
