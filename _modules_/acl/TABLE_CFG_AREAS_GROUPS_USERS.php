...
$user->values    = $tabla->toarray('users', ' SELECT user_id AS ID, CONCAT(username,\' -  \',user_fullname) AS NAME '
                                         ...
                                         . '     SELECT ID_AREA FROM CFG_AREAS_GROUPS WHERE ID = '.(int)$parent
                                         . '   ) '
                                         . ' ) '   
                                );      
...
class AREAS_GROUPS_USERS_Events extends defaultTableEvents implements iEvents{ 
    function OnCalculate($owner,&$row) { 
        $user = $owner->query2array('SELECT * FROM '.TB_USER.' WHERE user_id='.(int)$row['ID_USER']);
    ...
    function OnDrawCell($owner,&$row,&$col,&$cell){
        ...
        if(in_array($owner->state,array('update','insert'))){
            if($col->fieldname=='ID_USER') {
                // El AJAX de 'edit' no envía parent, así que $parent llega como 1 y la lista
                // se construiría con el área equivocada. Se recalcula con el parent real de la fila.
                $col->values = $owner->toarray('users', ' SELECT user_id AS ID, CONCAT(username,\' -  \',user_fullname) AS NAME '
                                                    . ' FROM '.TB_USER
                                                    . ' WHERE user_id IN ( '
                                                    . '   SELECT ID_USER FROM CFG_AREAS_USERS WHERE ID_AREA = ( '
                                                    . '     SELECT ID_AREA FROM CFG_AREAS_GROUPS WHERE ID = '.(int)$owner->parent_value
                                                    . '   ) '
                                                    . ' ) ' );
            }
        }
