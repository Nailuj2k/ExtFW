...
if (!Administrador())
  $tabla->where = 'ID IN (SELECT ID_APP
                          ...
                          AND ID_AREA IN ( SELECT ID_AREA
                                           ...
                                           WHERE ID_USER = '.(int)$_SESSION['userid'].'
                                           AND ADMIN = 1 ) )';
...
class myTableMysqlApps extends TableMysql{
    ...
    public function isAdmin(){
        ...
        $sql = 'SELECT ID_AREA FROM CFG_AREAS_APPS WHERE ID_APP = '.(int)$owner->parent_value.' AND ID_AREA IN ('
              .'SELECT ID_AREA FROM CFG_AREAS_USERS WHERE ID_USER = '.(int)$_SESSION['userid'].' AND ADMIN = 1 )';  // areas with this user is admin
