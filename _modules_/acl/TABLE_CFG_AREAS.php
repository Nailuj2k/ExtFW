...
if (!Administrador())
    $tabla->where = 'ID IN ( SELECT ID_AREA FROM CFG_AREAS_USERS WHERE ID_USER = '.(int)$_SESSION['userid'].' AND ADMIN = 1)';
...
class AREAS_Events extends defaultTableEvents implements iEvents{ 
    ...
    function update_members_from_roles($owner,&$result,$post) {
        ...
        $strSQL  = 'SELECT user_id AS id_user FROM '.TB_USER.' WHERE  user_id IN ('
                 . " SELECT id_user FROM ".TB_ACL_USER_ROLES." WHERE id_role = {$id_role} AND id_user NOT IN "
                 . "   (SELECT ID_USER FROM CFG_AREAS_USERS WHERE ID_AREA = {$id_area})"
                 . ') AND user_active="1"';
        $msg=$strSQL.'<br />';
        ...
        if(count($users)>0){
            foreach($users as $user){
                $owner->sql_query("INSERT INTO CFG_AREAS_USERS (ID_AREA,ID_USER) VALUES ({$id_area},".(int)$user['id_user'].")");
            }
