...
$tabla = new TableMysql(TB_PREFIX.'_TAGS');
...
class catStEvents extends defaultTableEvents implements iEvents{ 
    ...
    function OnDrawRow($owner,&$row,&$class){
        ...
        if($row['FILE_NAME'] && file_exists($filename)){
          $row['FILE_NAME']  =  "<a class=\"{$ext} swipebox framed\" rel=\"gallery\" href=\"{$row['IMAGES']['FILE_NAME']['URL']}\">"
      ...
      if($row['HEADER_FILE_NAME'] && file_exists($filename)){
          $row['HEADER_FILE_NAME']  =  "<a class=\"{$ext} swipebox framed\" rel=\"gallery\" href=\"{$row['IMAGES']['HEADER_FILE_NAME']['URL']}\">"
  ...
  function OnInsert($owner,&$result,&$post) { 
      ...
      if(!$post['NAME']) {
          ...
          $r = $owner->recordCount( 'SELECT TAG_ID FROM '.$owner->tablename.' WHERE NAME = \''.$post['NAME'].'\'');
          if($r >0) {
              $result['error'] =5;
  ...
  function OnUpdate($owner,&$result,&$post) { 
      ...
      if(!$post['NAME']) {
          ...
          $r = $owner->recordCount( 'SELECT TAG_ID FROM '.$owner->tablename.' WHERE NAME = \''.$post['NAME'].'\' AND TAG_ID <> '.$post['ID']);
          if($r >0) {
              $result['error'] =5;
              $result['msg'] = 'Ya existe una categoría con el identificador '.$post['NAME'];
          }else{
  ...
  function OnAfterUpdate($owner,&$result,&$post){
      if($post['fake_input_FILE_NAME']=='') Table::sqlExec('UPDATE '.$owner->tablename." SET FILE_NAME='' WHERE TAG_ID={$post['TAG_ID']}");
      if($post['fake_input_HEADER_FILE_NAME']=='') Table::sqlExec('UPDATE '.$owner->tablename." SET HEADER_FILE_NAME='' WHERE TAG_ID={$post['TAG_ID']}");
  }
    
  function OnAfterCreate($owner){ 
    if($owner->recordCount()<1){
      Table::sqlExec("INSERT INTO {$owner->tablename} (CAPTION,NAME,COLOR) VALUES('Etiqueta 1','tag_1','#33b075')");
      Table::sqlExec("INSERT INTO {$owner->tablename} (CAPTION,NAME,COLOR) VALUES('Etiqueta 2','tag_2','#ff0000')");
    }
  } 
