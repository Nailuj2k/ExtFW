...
// $type->values    = array('1'=>'Aviso normal','2'=>'Popup','3'=>'Imagen','4'=>'Redirect','5'=>'login');

if($TYPE=='1'){


    // Infinite Marquee Speed Control
    // https://codepen.io/t_afif/pen/QwGjxPv
    // https://codepen.io/t_afif/pen/RNGgPjy
                 ...
                 --><ul class="js-frame announcements ticker"><?php
                    foreach ($rows_newsticker as $row_newsticker){
                        $is_url = str_contains($row_newsticker['TEXT'], 'http');  //Check is url
                        ?>
                        <li class="js-item ticker__item"><span class="alert-type" title="<?=$row_newsticker['TEXT']?$row_newsticker['TEXT']:$row_newsticker['DESCRIPTION']?>"><?php

                           if($is_url)
                               echo '<a target="new"  style="font-size:1em;font-weight:bold;" href="'.$row_newsticker['TEXT'].'">'.Str::truncate($row_newsticker['DESCRIPTION'], 125).'</a>';
                           else 
                               echo Str::truncate($row_newsticker['DESCRIPTION'], 65)
                        ...
                        if($is_url){     
                           //$_SESSION[$popup_id]=false;
                            if (!$_SESSION[$popup_id]) {
                                ...
                                ?>
                                <style>
				.jux-dialog-overlay{padding:50px;}.jux-dialog-content{max-height:80% !important;margin:50px !important;max-width:700px !important;}
				/*.wq-dialog-content{max-height: 80%;max-width: 600px;}*/</style>
                                ...
                                <script type="text/javascript"> 
                                    $(function() {
                                      ...
                                      //OLD image('<?='media/CFG_ALERTS_FILES/files/'.$row_popup['ID'].'/'.$image_popup['FILE_NAME']?>','<?=$row_popup['NAME']?>',20000);
                                            image('<?=SCRIPT_DIR_MEDIA.'/CFG_ALERTS_FILES/files/'.$row_popup['ID'].'/'.$image_popup['FILE_NAME']?>',false,10000);
                                        },2000); 
                                    });
                                </script>
                                <?php
