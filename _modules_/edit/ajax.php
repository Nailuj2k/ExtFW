<?php
    ...
    function edit_safe_path(string $requested): string|false {
        $base = realpath(SCRIPT_DIR);
        // htaccess special case: resolve from project root
        $candidate = realpath(SCRIPT_DIR . '/' . $requested);
        if ($candidate === false) {
            // file may not exist yet for settext — check the directory at least
            $candidate = realpath(dirname(SCRIPT_DIR . '/' . $requested));
            if ($candidate === false || strpos($candidate, $base) !== 0) return false;
            return SCRIPT_DIR . '/' . $requested;
        }
        return strpos($candidate, $base) === 0 ? $candidate : false;
    }
    ...
    if       ($_ARGS['op']=='getfile'){
        ...
        $file = edit_safe_path($_ARGS['file']);
        if (!$file) { $ajax_result['error']=1; $ajax_result['msg']='Access denied'; echo json_encode($ajax_result); exit; }
      ...
      //$file     = Str::sanitizeName($_POST['filename'],true);
        ...
        $_safe_file = edit_safe_path($_ARGS['file']);
        if (!$_safe_file) { $ajax_result['error']=1; $ajax_result['msg']='Access denied'; echo json_encode($ajax_result); exit; }

        if (file_exists( $_safe_file ) ){
           if (is_writable ( $_safe_file ) ){
                ...
                if((!$_decrypted_text || $_decrypted_text=='' ) && strlen($_encrypted_text)>3){
                    ...
                    if ( copy($_safe_file, $bak_file ) ) {

                        $ret = file_put_contents($_safe_file, $text);
        ...
        define('CEREBRAS_API_KEY', CFG::$vars['ai']['cerebras']['api_key']);
       ...
       try {
           switch( $_ARGS['service'] ){
                ...
                case 'kimi'    :
                    ...
                    break;
                case 'cerebras':
                    include(SCRIPT_DIR_MODULE.'/ai/CerebrasAiService.php');
                    $aiService = new \AIServices\CerebrasAiService();
                    break;
                default        :
                    include(SCRIPT_DIR_MODULE.'/ai/DummyAiService.php');
                    $aiService = new \AIServices\DummyAiService();
