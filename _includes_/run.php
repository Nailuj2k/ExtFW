<?php
   ...
   // $_ARGS = array_merge( $_ARGS, $_REQUEST );
    ...
    // unset _GET && _POST here to avoid conflictos?
    // unset($_GET);
    // unset($_POST
    ...
    if($arg_0){   
        if (!file_exists(SCRIPT_DIR_MODULES.'/'.$arg_0)) {   // Is not MODULE
         ...
         ///}else if (  in_array(CFG::$vars['default_module'],array('page','blog','news','products','tienda','shop'))  ) { //FIX  check for 'page','blog', or any entries based module
            }else if ( CFG::$vars['default_module']) { // (  in_array(CFG::$vars['default_module'],array('page','blog','news','products','tienda','shop'))  ) { //FIX  check for 'page','blog', or any entries based module
    ...
    APP::setArgs($_ARGS);
    APP::$plugins->load_plugins();      // antes de auth.php para que los plugins puedan capturar 'after_auth'
    ...
    // hooks.php del theme: solo registra callbacks; va antes del init.php del theme
    // para que pueda capturar 'after_theme_init'
    ...
    ////////include( SCRIPT_DIR_MODULE   . '/i18n.php'); // aqui no !!! :)
    include( SCRIPT_DIR_MODULE   . '/init.php' );
    include( SCRIPT_DIR_CLASSES  . '/' . $db_engine . '/init.php');

    // Rate limit global. Va DESPUÉS del init de $db_engine a propósito: ese init carga
    // CFG_CFG en CFG::$vars, y el limiter lee su config de CFG::$vars['ratelimit']
    // (dry_run/max_user/max_anon, editables en Control Panel → Ajustes) sin queries propias.
    // El límite específico de BTCPay permanece en _modules_/noxtr/raw.php.
    include_once(SCRIPT_DIR_INCLUDES . '/ratelimiter.php');
    
    CFG::$vars['site']['langs']['enabled'] = $cfg['enable_langs'] ?? false;

    Hook::do_action('after_module_init');       // módulo inicializado, $db_engine listo
   
    define('VERSION', CFG::$vars['script']['version']);  
    
    Captcha::$innumerate = CFG::$vars['captcha']['innumerate'] ?? false;
