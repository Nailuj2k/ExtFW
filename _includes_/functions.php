...
function t($s,$c1='',$c5='',$insert=false){
    ...
    if($_SESSION['translate']){
        if ( ctype_upper($s) || strpos($s, '_') ||strpos($s, '.') || strpos($s, '%s')  || strpos($s, '{')  || strpos($s, '}')  || strpos($s, '$') ){
            ...
            $rows_oki = Table::sqlQueryPrepared("SELECT str_id,str_string FROM ".TB_STR." WHERE str_string = ?", [$s]);
            $oki = $rows_oki ? $rows_oki[0] : false;
            if(!$oki) Table::sqlQueryPrepared("INSERT INTO ".TB_STR." (str_string) VALUES (?)", [$s]);

            $rows_id = Table::sqlQueryPrepared("SELECT str_id FROM ".TB_STR." WHERE str_string = ?", [$s]);
            $_id = $rows_id ? $rows_id[0] : false;
            if($_id) {
                $id  = $_id['str_id'];

                $rows_cc1 = Table::sqlQueryPrepared("SELECT id_str FROM ".TB_CC." WHERE id_lang=1 AND id_str = ?", [$id]);
                $rows_cc5 = Table::sqlQueryPrepared("SELECT id_str FROM ".TB_CC." WHERE id_lang=5 AND id_str = ?", [$id]);
                $_cc1 = $rows_cc1 ? $rows_cc1[0] : false;
                $_cc5 = $rows_cc5 ? $rows_cc5[0] : false;

                if($_cc1) $cc1 = $_cc1['id_str'];
                if($_cc5) $cc5 = $_cc5['id_str'];

                $s_default = str_replace('_',' ',ucwords(strtolower($s)));
                if     ($_id && !$cc1 && $c1!='') Table::sqlQueryPrepared("INSERT INTO ".TB_CC." (id_str,id_lang,cc_string) VALUES (?,1,?)", [$id,$c1]);
                else if($_id && !$cc1 && $c1=='') Table::sqlQueryPrepared("INSERT INTO ".TB_CC." (id_str,id_lang,cc_string) VALUES (?,1,?)", [$id,$s_default]);
                if     ($_id && !$cc5 && $c5!='') Table::sqlQueryPrepared("INSERT INTO ".TB_CC." (id_str,id_lang,cc_string) VALUES (?,5,?)", [$id,$c5]);
                else if($_id && !$cc5 && $c5=='') Table::sqlQueryPrepared("INSERT INTO ".TB_CC." (id_str,id_lang,cc_string) VALUES (?,5,?)", [$id,$s_default]);
            }            
...
function get_page($name){
    global $_ACL;
    $rows = Table::sqlQueryPrepared("SELECT * FROM ".TB_PAGES." WHERE item_name = ?", [$name]);
