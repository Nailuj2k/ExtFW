<?php
  ...
  //              HTML::css(SCRIPT_DIR_LIB.'/file_viewer/file_viewer.css?v='.VERSION);
        ...
        <?php
        // Serializa textos PHP para incrustarlos como literales JavaScript válidos.
        // Es necesario, entre otros casos, cuando una traducción contiene apóstrofes.
        if (!function_exists('t_js')) {
            function t_js($s, $c1='', $c5='', $insert=false) {
                return json_encode(t($s, $c1, $c5, $insert), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
            }
        }
        ?>
        ...
        <script>
            ...
            const str_of        = <?=t_js('OF')?>;
            const str_row       = <?=t_js('ROW','fila')?>;
            const str_item      = <?=t_js('ITEM')?>;
            const str_copy      = <?=t_js('COPY','Copiar')?>;
            const str_page      = <?=t_js('PAGE')?>;
            const str_vote      = <?=t_js('VOTE','voto','vote')?>;
            const str_stars     = <?=t_js('STARS','estrellas','stars')?>;
            const str_votes     = <?=t_js('VOTES','votos','votes')?>;
            const str_price     = <?=t_js('PRICE')?>;
            const str_accept    = <?=t_js('ACCEPT','Aceptar')?>;
            const str_cancel    = <?=t_js('CANCEL','Cancelar')?>;
            const str_delete    = <?=t_js('DELETE')?>;
            const str_no_rating = <?=t_js('NO_RATING','Sin calificación','No rating')?>;

            const str_delete_image = <?=t_js('DELETE_IMAGE','Eliminar imagen')?>;
            const str_reset_styles = <?=t_js('RESET_STYLES','Eliminar estilos')?>;
            const str_edit_file    = <?=t_js('EDIT_FILE','Editar archivo')?>;
            const str_delete_file  = <?=t_js('DELETE_FILE','Eliminar archivo')?>;
            const str_edit         = <?=t_js('EDIT','Editar')?>;
            const str_add_files    = <?=t_js('ADD_FILES','Añadir archivos')?>;

            const str_sign_passwordless = <?=t_js('SIGN_PASSWORDLESS','Prefiero usar acceso sin contraseña')?>;
            const str_sign_password     = <?=t_js('SIGN_IN_WITH_PASSWORD','Prefiero usar contraseña')?>;
        ...
        <?php 
        include(SCRIPT_DIR_MODULES.'/'.MODULE.'/i18n.php');  
        ?>
