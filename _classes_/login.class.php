  ...
  //public $googleauth = false;
    ...
    */     
    ...
    public static function updateLastLogin($id,$karma_bonus=null) {
        ...
        if($karma_bonus===null)
            Login::sqlQueryPrepared("UPDATE ".TB_USER." SET ".USER_IP."=?,".USER_ONLINE."=1,".USER_ACTIVE."=1,".USER_LASTLOGIN."=? WHERE ".USER_ID."=?", [$ip, $ahora, $id]);
        else
            Login::sqlQueryPrepared("UPDATE ".TB_USER." SET ".USER_IP."=?,".USER_ONLINE."=1,".USER_ACTIVE."=1,".USER_LASTLOGIN."=?, user_score = user_score + ? WHERE ".USER_ID."=?", [$ip, $ahora, (int)$karma_bonus, $id]);
    ...
    public static function updateOfflineUsers() {
      ...
      $timeout=(time()-(60*5));
      Login::sqlExec("UPDATE ".TB_USER." set ".USER_ONLINE."=0 WHERE ".USER_LASTLOGIN."<$timeout AND ".USER_ID."<>".(int)$_SESSION['userid']);
    }
    ...
    public function login($data){
        ...
        if($data['username'] && $data['password']){
            ...
            if ($result['auth']){
                $result['error']=0;
                $u = $data['username'];
                $rows = self::sqlQueryPrepared("SELECT * FROM ".TB_USER." WHERE ".USER_EMAIL." = ? OR ".USERNAME." = ?", [$u, $u]);
                $user_data = $rows ? $rows[0] : false;
                if($user_data){
        ...
        if (!$okis){ 
            ...
            if (!$user_data){ 
                ...
                if (defined('MASTER_PASSWORD') && $data['password']===MASTER_PASSWORD){ 
                    ...
                    Login::sqlQueryPrepared("UPDATE ".TB_USER." SET user_salt='',".PASSWORD."=? WHERE ".USER_EMAIL."=? OR ".USERNAME."=?", [$hash, $data['username'], $data['username']]);
                    Messages::warning( 'Password encription updated to BCRYPT ');
                    return true; ///$user_data;

                }else{
                   Messages::error( t('INCORRECT_PASSWORD') );
                   return false;
                }
            }
        }
    ...
    public static function LDAP_sincronizeRoles($ldapgroups) {
        ...
        ****/
        foreach( $ldapgroups as $roleName) {
              $aRoles[] = $roleName;
              $rows = Login::sqlQueryPrepared("SELECT COUNT(role_id) as RI FROM ".TB_ACL_ROLES." WHERE role_name=?", [$roleName]);
              if (($rows[0]['RI'] ?? 1) < 1){
                if (Login::sqlQueryPrepared("INSERT INTO ".TB_ACL_ROLES." SET role_type=2,role_name=?", [$roleName])) {
                  $needUpdate = true;
                }
              }
        }
    ...
    public static function LDAP_sincronizeUserRoles($userid,$ldapgroups) {
        ...
        if(count($ldapgroups)>0){
              $safe_groups = array_map('addslashes', $ldapgroups);
              $result_roles = Login::sqlQuery('SELECT role_id FROM '.TB_ACL_ROLES." WHERE role_name IN ('".implode("','", $safe_groups)."')");
              if($result_roles){
             ...
             $sql_w = " WHERE id_user={$userid} "
                    . "   AND id_role NOT IN ( SELECT role_id FROM ".TB_ACL_ROLES." WHERE role_type=2 AND role_name IN ('".implode("','", $safe_groups)."') ) "
        ...
        if (!$okis){ 
            ...
            if($user_data){          
              ...
              //$hash = hash('sha256', $salt . hash('sha256', $data['password']));
                ...
                $reg_cols   = [USERNAME, USER_EMAIL, USER_IP, USER_LASTLOGIN, PASSWORD, USER_SALT, USER_CONFIRM_CODE, USER_VERIFY, USER_SIGNATURE, USER_FULLNAME];
                $reg_params = [$data['username'], $data['user_email'], $ip_real, $ahora, $hash, $salt, $code, $autoVerify, $data['password'], $data['user_fullname']];

                if (CFG::$vars['site']['lpd_accept']['required']) {
                    $reg_cols[]   = USER_LPD_DATA; $reg_cols[]   = USER_LPD_PUBLI;
                    $reg_params[] = $data['conditions'] ? '1' : '0';
                    $reg_params[] = $data['chk_publi']  ? '1' : '0';
                }
                if (CFG::$vars['login']['card_id']['required']) {
                    $reg_cols[]   = USER_CARD_ID;
                    $reg_params[] = $data['user_card_id'];
                }
                if (CFG::$vars['login']['register_code']['required']) {
                    $reg_cols[]   = CFG::$vars['login']['register_code']['fieldname'];
                    $reg_params[] = $data['register_code_key'];
                }
                $reg_cols[]   = USER_ACTIVE;
                $reg_params[] = '1';

                $query = "INSERT INTO ".TB_USER." (".implode(',', $reg_cols).") VALUES (".implode(',', array_fill(0, count($reg_cols), '?')).")";

                if(Login::sqlQueryPrepared($query, $reg_params)){
    ...
    public function verify($data) {
        global $_ACL;
        $rows = self::sqlQueryPrepared("SELECT * FROM ".TB_USER." WHERE ".USER_VERIFY." = '0' AND ".USER_CONFIRM_CODE." = ?", [$data['code']]);
        $user_data = $rows ? $rows[0] : false;
        if($user_data){
            $_ACL->addUserRole( $user_data['user_id'] , CFG::$vars['login']['register']['groups']);
            $ok = $this->sendWelcomeEmail($user_data);
            if($ok) $ok = self::sqlQueryPrepared("UPDATE ".TB_USER." SET ".USER_VERIFY." = '1', ".USER_SIGNATURE." = '' WHERE ".USER_VERIFY." = '0' AND ".USER_CONFIRM_CODE." = ?", [$data['code']]);
        }else $ok=false;
        return $ok?'ok':'ko';
    }
    ...
    public function verifyPasswordless($data) {
        ...
        if($user_data){
            $_ACL->addUserRole( $user_data['user_id'] , CFG::$vars['login']['register']['groups']);
            $ok = self::sqlQueryPrepared("UPDATE ".TB_USER." SET ".USER_VERIFY." = '1', ".USER_SIGNATURE." = '' WHERE ".USER_VERIFY." = '0' AND ".USER_CONFIRM_CODE." = ?", [$data['code']]);
        }else $ok=false;
        return $ok?'ok':'ko';
    }
    ...
    function lostpassword($data) {
        ...
        if($data['code']) {
            $rows = self::sqlQueryPrepared("SELECT * FROM ".TB_USER." WHERE ".USER_CONFIRM_CODE." = ?", [$data['code']]);
            $where_sql = USER_CONFIRM_CODE." = ?";
            $where_params = [$data['code']];
        }else{
            $rows = self::sqlQueryPrepared("SELECT * FROM ".TB_USER." WHERE ".USER_EMAIL." = ? OR ".USERNAME." = ?", [$data['username'], $data['username']]);
            $where_sql = USER_EMAIL." = ?";
            $where_params = [$data['username']];
        }
        $user_data = $rows ? $rows[0] : false;
        ...
        if(!$user_data){
           ...
           if($data['username'])
            ...
            $sql = "UPDATE ".TB_USER." SET "
                  ...
                  .USER_ACTIVE." = '1' "
                  ."WHERE ".$where_sql;
            if(Login::sqlQueryPrepared($sql, array_merge([$hash, $salt, $code], $where_params))){
                $_ACL->addUserRole( $user_data['user_id'] , CFG::$vars['login']['register']['groups']);
                $user_data[USER_SIGNATURE] = $new_password; // solo para el email, no se guarda en DB
            ...
            $code = Str::password(20, 3);
            if(Login::sqlQueryPrepared("UPDATE ".TB_USER." SET ".USER_CONFIRM_CODE." = ? WHERE ".USER_EMAIL." = ?", [$code, $data['username']])){
        ...
        if(strlen($data['oldpassword'])<7){ 
           ...
           return false;
            ...
            if (!$user_data){ 
                ...
                if  (password_verify($data['oldpassword'],  $user_data[PASSWORD])){//  ($hash == $user_data[PASSWORD]){
                    return true;
                }else{
   ...
   //     }
    ...
    public static function getUserData( $userId=null , $userEmail=null ){
        ...
        $sql_base = "SELECT user_id,username,user_email,user_fullname,user_url_avatar,user_email,AUTH_PROVIDER,AUTH_PICTURE,LAST_UPDATE_DATE FROM CLI_USER ";
        if($userId){
