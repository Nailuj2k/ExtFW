<?php
    ...
    class Location {
        ...
        public static function geoplugin_details(){
            ...
            $ip = self::ip();
            $details = file_get_contents('https://www.geoplugin.net/json.gp?ip='.$ip);
            ...
            if($details){
                ...
                $geoplugin_details = '{
                      ...
                      "region": "'.($obj->geoplugin_regionName ?? '').', '.($obj->geoplugin_region ?? '').'",
                     "country": "'.($obj->geoplugin_countryCode ?? '').'",
                         "loc": "'.($obj->geoplugin_latitude ?? '').','.($obj->geoplugin_longitude ?? '').'",
                      "postal": "'.($obj->geoplugin_areaCode ?? '').'",
                    "timezone": "'.($obj->geoplugin_timezone ?? '').'"
                }';
