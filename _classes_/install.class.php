...
class Install{
    ...
    public static function runsql($sql){
        ...
        // ejemplo de CREATE TABLE para SQLite:
        /*
        KO:
        $sqls[] = "CREATE TABLE `TGRAM_TEXTS` (
            `id` int(5) unsigned NOT NULL auto_increment,
            `keywords` text,
            `response` text,
            `priority` int(5),
            `active` int(1),
            `created_at` int(15) DEFAULT unix_timestamp(),
            PRIMARY KEY (`id`)
        ) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci";
        ...
        */
        ...
        if (CFG::$vars['db']['type']=='sqlite' && str_starts_with(trim($sql),'INSERT')){
            // El backup escapa las comillas simples con addslashes() (estilo MySQL: \').
            // SQLite no soporta el escape con backslash; requiere comillas dobladas ('').
            $sql = str_replace("\\'", "''", $sql);
        }
        ...
        foreach ($lines as $line){
           ...
           // if (CFG::$vars['db']['type']=='sqlite' && $line!='' && str_starts_with(trim($line),'PRIMARY KEY')!==false)  $line = ''."\n";
        ...
        echo 'Ejecutando <span style="color:#3399CC;font-size:8px;">'.mb_strimwidth($sql, 0, 300, "...").' ....</span><br />';
        $res=Table::sqlExec($sql);
        echo $res ? '  <span style="color:green;font-size:8px;"><b>OK</b></span><br />' 
                  : '  <span style="color:red;font-size:8px;"><b>ERROR '.Table::lastError().'</b><br>'.$sql.'</span><br>';  
