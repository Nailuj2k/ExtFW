...
class ACL extends BaseACL { 
    ...
    function getPermIdFromKey($permKey){  //COPY
        if(self::$cache === true && self::$session['permissions']) {
            ...
            $rows = self::sqlQueryPrepared("SELECT permission_id FROM ".TB_ACL_PERMISSIONS." WHERE permission_key = ? LIMIT 1", [$permKey]);
            return $rows ? $rows[0]['permission_id'] : false;
        }
    }
    ...
    public function getRoleIdFromName($roleName){
        if(self::$cache === true && self::$session['roles']) {
            ...
            $rows = self::sqlQueryPrepared("SELECT role_id FROM ".TB_ACL_ROLES." WHERE role_name = ? LIMIT 1", [$roleName]);
            return $rows ? $rows[0]['role_id'] : false;
        }
    }
    ...
    function getUserRoles($format='ids',$mask=''){
        ...
        //CFG    }
        //CFG}else{
            $where = $mask != '' ? " AND id_role IN (SELECT role_id FROM ".TB_ACL_ROLES." WHERE role_name LIKE ?)":'';
            $params = $mask != '' ? [(int)$this->userID, $mask] : [(int)$this->userID];
            $strSQL = "SELECT * FROM ".TB_ACL_USER_ROLES." WHERE id_user = ? " . $where . " ORDER BY user_role_add_date ASC";
            //print_r($strSQL); 
            $rows = self::sqlQueryPrepared($strSQL, $params);
            foreach($rows as $row){
                //$resp[] = $row['id_role'];
                if ($format == 'full'){
    ...
    function getAllRoles($format='ids')  {
        ...
        if(self::$cache === true && self::$session['roles']){
            ...
            $params = [];
            if(!empty($_SESSION['org_id'])) { $strSQL .= " WHERE org_id = ?"; $params[] = (int)$_SESSION['org_id']; }
            $strSQL .= " ORDER BY role_name ASC";
            $rows = self::sqlQueryPrepared($strSQL, $params);       //session roles
            foreach($rows as $row){      
                if ($format == 'full'){
    ...
    function getUserId($username){
      $data = self::sqlQueryPrepared("SELECT user_id FROM ".TB_USER." WHERE username = ? LIMIT 1", [$username]);
    ...
    function addUserRole($user_id,$role_name) {
      ...
      $rows = self::sqlQueryPrepared("SELECT role_id FROM ".TB_ACL_ROLES." WHERE role_name = ? LIMIT 1", [$role_name]);
      $role_id = $rows ? $rows[0]['role_id'] : false;
      if(!$role_id) {
        if (self::sqlQueryPrepared("INSERT INTO ".TB_ACL_ROLES." (role_name) VALUES(?)", [$role_name])) {
          $rows = self::sqlQueryPrepared("SELECT role_id FROM ".TB_ACL_ROLES." WHERE role_name = ? LIMIT 1", [$role_name]);
          $role_id = $rows ? $rows[0]['role_id'] : false;
        }
      }
      ...
      if($role_id){
        $rows = self::sqlQueryPrepared("SELECT count(0) AS n FROM ".TB_ACL_USER_ROLES." WHERE id_user=? AND id_role=?", [(int)$user_id, (int)$role_id]);
        $role_user = $rows ? $rows[0]['n'] : 0;
        if($role_user<1){
          self::sqlQueryPrepared("REPLACE INTO ".TB_ACL_USER_ROLES." SET id_user=?, id_role=?, user_role_add_date=?", [(int)$user_id, (int)$role_id, date("Y-m-d H:i:s")]);
        }
      }
      return true;
    }
    ...
    function deleteUserRole($user_id,$role_name) {
      ...
      self::sqlQueryPrepared("DELETE FROM ".TB_ACL_USER_ROLES." WHERE id_user=? AND id_role=?", [(int)$user_id, (int)$role_id]);
    }
    ...
    function deleteRole($role_name) {
      if(!$role_name) return false;
      return self::sqlQueryPrepared("DELETE FROM ".TB_ACL_ROLES." WHERE role_name = ?", [$role_name]);
    }
    ...
    function addPermission($perm_key,$perm_name='',$debug=false) {
      ...
      $rows = self::sqlQueryPrepared("SELECT permission_id FROM ".TB_ACL_PERMISSIONS." WHERE permission_key = ? LIMIT 1", [$perm_key]);
      $perm_id = $rows ? $rows[0]['permission_id'] : false;
      if(!$perm_id) {
        if (self::sqlQueryPrepared("INSERT INTO ".TB_ACL_PERMISSIONS." (permission_key,permission_name) VALUES(?,?)", [$perm_key,$perm_name])) {
          $rows = self::sqlQueryPrepared("SELECT permission_id FROM ".TB_ACL_PERMISSIONS." WHERE permission_key = ? LIMIT 1", [$perm_key]);
          $perm_id = $rows ? $rows[0]['permission_id'] : false;
        }
        if($debug) Vars::debug_var(__LINE__.'::addPermission::perm_id:'.$perm_id);
      }
      return $perm_id;
    }
    ...
    function addRolePerm($role_name,$perm_key,$debug=false) {
      ...
      $rows = self::sqlQueryPrepared("SELECT role_perm_id FROM ".TB_ACL_ROLE_PERMS." WHERE id_permission=? AND id_role=? LIMIT 1", [(int)$perm_id,(int)$role_id]);
      $role_perm_id = $rows ? $rows[0]['role_perm_id'] : false;

      if(!$role_perm_id) {
        $sql_insert = "INSERT INTO ".TB_ACL_ROLE_PERMS." (id_permission,id_role,role_perm_value) VALUES(?,?,1)";
        if (self::sqlQueryPrepared($sql_insert, [(int)$perm_id,(int)$role_id])) {
          $rows = self::sqlQueryPrepared("SELECT role_perm_id FROM ".TB_ACL_ROLE_PERMS." WHERE id_permission=? AND id_role=? LIMIT 1", [(int)$perm_id,(int)$role_id]);
          $role_perm_id = $rows ? $rows[0]['role_perm_id'] : false;
        }
