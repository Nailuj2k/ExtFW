   ...
   // $sql .= "  t.item_id NOT IN (SELECT DISTINCT(id_item) FROM ACL_ITEM_ROLES) ";
      ...
      if (isset($_SESSION['userid']) && $_SESSION['userid']>0) {
        ...
        $sql .= " t.item_id in(SELECT id_item FROM ACL_ITEM_ROLES "
              . " WHERE id_item=t.item_id AND id_role IN (SELECT id_role FROM ACL_USER_ROLES WHERE id_user=".(int)$_SESSION['userid']."))";
      }
     ...
     // $userroles = $useritemACL->getUserRoles();
      ...
      if($rows){
        foreach($rows as $row){      
          ...
          $this->items[$row['id']]['caption']=$row['caption']; //t($row['caption']);
          /**/
