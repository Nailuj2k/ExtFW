...
*/
    ...
    */
    ...
    // Registrar método dinámico
    public function addMethod(string $name, callable $callback): void {
        $this->dynamicMethods[$name] = $callback;
    }

    // Comprobar si existe un método dinámico registrado
    public function hasDynamicMethod(string $name): bool {
        return isset($this->dynamicMethods[$name]);
    }

    // __call captura métodos inexistentes
    ...
    public function setParent($fieldname,$value){
      global $_ARGS;
      $value = max(0,(int)$value);           // 0 = sin fila maestra seleccionada (evita generar JS inválido)
      $this->parent_key = $fieldname;        // Foreing Key for act as detail table - false or fieldname
      $this->parent_value = $value;  // Foreing Key value for act as detail table - false or field value
    ...
    */
    ...
    private function renderForm(&$form,$id=false){
      ...
      */
      ...
      Hook::do_action('on_before_show_form', $this, $form, $id);
      Hook::do_action('on_before_show_form_' . $this->tablename, $this, $form, $id);
      ...
      $form->render(true);
      Hook::do_action('on_after_show_form', $this, $form, $id);
      Hook::do_action('on_after_show_form_' . $this->tablename, $this, $form, $id);
    ...
    public function update( $post ) {
      ...
      if($result['old']===false){
          ...
          $hook_args = [$this, &$post];   // $post por referencia: los filtros pueden modificarlo
          $result = Hook::apply_filters('on_update', $result, $hook_args);
          $result = Hook::apply_filters('on_update_' . $this->tablename, $result, $hook_args);
      ...
      if($result['error']==0){
        ...
        if ($this->perms['edit']){
          if($this->sql_exec($sql)){
             ...
             $after_args = [$this, $post];   // aquí $post va por valor: la fila ya está actualizada
             $result = Hook::apply_filters('on_after_update', $result, $after_args);
             $result = Hook::apply_filters('on_after_update_' . $this->tablename, $result, $after_args);
    ...
    */
    ...
    public function show(){
      ...
      */
      ...
      Hook::do_action('on_show', $this);
      Hook::do_action('on_show_' . $this->tablename, $this);
