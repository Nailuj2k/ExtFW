...
//  public $markup = '<td>%s</td>';
   ...
   // public function pk($v)          { $this->pk            = $v; $this->editable=false; return $this; }
    ...
    public function get($keyValue) {
      return getFieldValue($this->tablename,$this->fieldname,' WHERE '.$this->keyName.'='.(int)$keyValue,true,true);
    }
    ...
    public function getFormElement($value=false) {
      ...
      switch($this->type) {
        ...
        case 'datetime':
          ...
          break;
        case 'unixtime':
          return new formInput($this,time()); //new formInputDateTime($this,$value); //($this->name,$this->label,$value); 
          break;
        case 'color':
