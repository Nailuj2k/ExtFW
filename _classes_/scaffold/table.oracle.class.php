  ...
  class TableOracle extends Table{
    ...
    function __construct($tablename=false,$external=false){
      ...
      if (!is_resource($this->resource) && !is_object($this->resource)) {
        throw new RuntimeException('No se pudo establecer la conexión con Oracle para el módulo '.MODULE.'.');
      }

      // El constructor padre puede ejecutar sql_query(). La conexión debe
      // existir antes de llamar al constructor padre.
...
/*
    ...
    */
    public function sql_query($sql){
      ...
      //$this->debug( $sql , true);
      if (!is_resource($this->resource) && !is_object($this->resource)) {
        $this->error = 'ERROR: no hay una conexión Oracle disponible.';
