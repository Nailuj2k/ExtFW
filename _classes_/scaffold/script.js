...
// Devuelve el fragmento '/parent=N' de una tabla detalle (vacío si no lo es).
// Sin él, el TABLE_*.php se incluye sin saber a qué fila maestra pertenece.
function parentArg(T) {
  var X = window['X_' + T];
  return (X && X['parent']) ? '/parent=' + X['parent'] : '';
}
...
function editRow(tablename, id, module = null, page = null) {   
    ...
    var url = AJAX_URL + '/' + module + '/ajax/module=' + module + '/op=edit/table=' + tablename + '/id=' + id + '/page=' + page + parentArg(tablename);
...
$('body').on('dblclick', '.datatable table td', function (e) {
  ...
  if ($(this).hasClass('editable') || $(this).hasClass('editable-select')) {
    ...
    if (Row.hasClass('edit')) {
      ...
      var url = AJAX_URL + '/' + module_name + '/ajax/module=' + module_name + '/op=view/table=' + T + '/id=' + rowId + parentArg(T);
...
$('body').on('click', '.datatable .actions a.delete', function () {
  ...
  $.modalform({'title':'Eliminar fila', 'html': '<h4>Va a eliminar esta entrada ¿está seguro? </h4><br/ >Esta acción no podrá deshacerse.', 'buttons': 'ok cancel' }, function (accept) {
    if (accept) {
      ...
      $.get(AJAX_URL + '/' + module_name + '/ajax/module=' + module_name + '/op=delete' + '/table=' + T + '/id=' + rowId + parentArg(T), function (data) {
...
$('body').on('click', '.datatable a.setup', function (e) {
  var T = $(this).closest('.datatable').attr('id').replace('T-', '');
  var X = window['X_' + T]
  var url = AJAX_URL + '/' + module_name + '/ajax/module=' + module_name + '/op=setup/table=' + T + parentArg(T);
...
$('body').on('click', '.datatable a.print', function (event) {
  var T = $(this).closest('.tb_id').attr('id');
  var X = window['X_' + T];
  var Row = (X['output'] == 'table') ? $('#' + T + ' tr.active') : $('#' + T + ' .div_item.active');
  var rowId = Row.attr('id').replace('row-', '');
...
//  ++++++ click row +++++++++
$('body').on('click', ' .datatable .div_item, .datatable table .table-row td', function () {
...
$('.div_item').on('click', 'dt', function () {
  ...
  if (Row.attr('id') == undefined) {
    ...
    if (Row.attr('id')) {
      ...
      //var X =  eval('X_'+T);
      var url = AJAX_URL + '/' + module_name + '/ajax/module=' + module_name + '/op=edit/table=' + T + '/id=' + rowId + '/page=' + X['page'] + parentArg(T);
      ...
      if (Row.hasClass('row_editing_on')) {
        ...
        if (X['output'] == 'table' && selected_table && selected_table_id) {
          ...
          if (R.hasClass('edit')) {
            ...
            //alert(R.attr('id'));
            var url = AJAX_URL + '/' + module_name + '/ajax/module=' + module_name + '/op=view/table=' + selected_table + '/id=' + selected_table_id + parentArg(selected_table);
            if (X['detail']) $.modalform({ 'url': url, 'buttons': 'ok' });
          }
          //++new to here
        }//else
