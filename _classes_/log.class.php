...
class LOG {
    ...
    /**
     * Log rate limiter events (mismo patrón que security()): una línea JSON por evento,
     * archivo diario rate_YYYYMMDD.log en SCRIPT_DIR_LOG (rotación diaria implícita).
     *
     * @param string $type 'dryrun-block' (habría bloqueado, modo simulación),
     *                     'block' (bloqueo real 429) o 'low' (quedan pocos tokens)
     * @param array  $data key, remaining, limit, module, action...
     */
    public static function rate(string $type, array $data = []) {
        ...
        $log_filename = SCRIPT_DIR_LOG . '/rate_' . date('Ymd') . '.log';
