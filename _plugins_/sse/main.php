...
document.addEventListener('DOMContentLoaded', function(){
    ...
    const sse_{$id} = new SSEClient({
        ...
        onMessage: function(data) {
            if (data.kind === 100 && data.payload && data.payload.action === 'clear') {
                document.getElementById('{$id}_msgs').innerHTML = '';
                return;
            }
            renderMsg(data, true);
        },
    ...
    async function mcCommand(cmd) {
        const name = cmd.slice(1).split(/\\s+/)[0].toLowerCase();
        if (name === 'clear') {
            try {
                const r = await fetch("{$url}/ajax/op=clear", { method: 'POST' });
                const j = await r.json();
                if (j.error > 0) { mcSystem(j.msg || 'No autorizado'); return; }
                document.getElementById('{$id}_msgs').innerHTML = '';
            } catch (e) {
                mcSystem('Error al ejecutar el comando');
            }
        } else {
            mcSystem('Comando desconocido: /' + name);
        }
    }
        ...
        .then(function(data) {
            if (data.error === 0 && data.messages) {
                data.messages.forEach(function(msg) {
                    if (msg.kind === 100 && msg.payload && msg.payload.action === 'clear') return;
                    renderMsg(msg, false);
                });
            }
        });
    ...
    btn.addEventListener('click', async function() {
        ...
        const result = await sse_{$id}.sendMessage(1, input.value, {avatar: "{$avatar}"});
