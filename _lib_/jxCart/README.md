...
Forked from simpleCart(c) 2017 Nailuj
...
To get started, just add the jxCart javascript file to your page, and set your PayPal checkout:

	<script src="jxCart.js"></script>
	<script>
		jxCart({
			checkout: { 
				type: "PayPal" , 
...
Please check out our documentation to see all of the options jxCart has available!
...
##jxCart(js) Setup/Initialization
	
jxCart(js) _requires using jQuery, Prototype, or Mootools_. No extra configuration 
is needed as long as one of those libraries is included on the page
	
You can set/change jxCart options at any time:
...
Here are the possible options and their default values: 
	...
	jxCart({
		...
		// how jxCart should checkout, see the checkout reference for more info 
		checkout: { 
			type: "PayPal" , 
			email: "you@yours.com" 
		},
...
You can make items be available to your users by simple using class names in your html. For any Item you want to be available to be added to the cart, you make a container with a class name of `jxCart_shelfItem`. Then add classes to tags inside of that container that have the general form `item_[name of field]` and jxCart will use the value or innerHTML of that tag for the cart. For example, if you wanted to sell a T-shirt with 3 different sizes, you can do this:
...
**For more information, please go to jxCartjs.com**								  
