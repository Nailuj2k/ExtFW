const barcode = function () {
    ...
    function init() {
		...
		function comenzarTrasmision(stream){
			elements.video.srcObject = stream;
			elements.currentStream = stream;
		}
        ...
        select.addEventListener('change', event => {
            if (typeof elements.currentStream !== 'undefined') {
                stopMediaTracks(elements.currentStream);
            }
            if (select.value === '') {
    ...
    function stopMediaTracks(stream = null) {
        if (!stream)
            stream = elements.currentStream;
