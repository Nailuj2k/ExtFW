...
class Ceca{
	...
	/**
	 * @var integer $_inicioBizum Opcional 1 Usar Bizum.
	 * 
	 * 0/1. Indica si se va directamente a Bizum sin pasar por 
     * la pantalla de petición de selección de forma de pago. 
     *
	 */
	protected $_inicioBizum;
	...
	public function __construct()
	{
		...
		$this->_inicioBizum = 0; // Por defecto NO Bizum
	}
    ...
    /**
     * Inicio Bizum
     * @param integer $value Si 1 se presenta directamente la pantalla de pago con Bizum.
     */
	public function setInicioBizum($value)
	{
		$this->_inicioBizum = $value;
	}
	...
	*/
    ...
    public function setNameform($nombre = 'ceca_form')
    {
    ...
    public function setIdform($idform = 'ceca_form')
    {
        $this->_idForm = $idform;
    }
    ...
    */
	...
	public function create_form(){
        ...
        <form action="'.$this->_urlPasarela.'" method="post" id="'.$this->_idForm.'" name="'.$this->_nameForm.'" enctype="application/x-www-form-urlencoded" >
            <input type="'.$type.'" name="MerchantID" value="'.$this->_merchantID.'" />
            <input type="'.$type.'" name="AcquirerBIN" value="'.$this->_acquirerBIN.'" />
            <input type="'.$type.'" name="TerminalID" value="'.$this->_terminalID.'" />
            <input type="'.$type.'" name="URL_OK" value="'.$this->_url_ok.'" />
            <input type="'.$type.'" name="URL_NOK" value="'.$this->_url_nok.'" />
            <input type="'.$type.'" name="Firma" value="'.$this->_firma.'" />
            <input type="'.$type.'" name="Cifrado" value="'.$this->_cifrado.'" />
            <input type="'.$type.'" name="Num_operacion" value="'.$this->_num_operacion.'" />
            <input type="'.$type.'" name="Importe" value="'.$this->_importe.'" />
            <input type="'.$type.'" name="TipoMoneda" value="'.$this->_tipoMoneda.'" />
            <input type="'.$type.'" name="Exponente" value="'.$this->_exponente.'" />
            <input type="'.$type.'" name="Pago_soportado" value="'.$this->_pago_soportado.'" />
            <input type="'.$type.'" name="Descripcion" value="'.$this->_descripcion.'" />
            <input type="'.$type.'" name="Idioma" value="'.$this->_idioma.'" />            
            <input type="'.$type.'" name="inicioBizum" value="'.$this->_inicioBizum.'" />            
        ';
        //	$formulario.=$this->_submit;
        	$formulario.='
        </form>        
        ';
        $firma =  $this->_clave_encriptacion . $this->_merchantID . $this->_acquirerBIN . $this->_terminalID . $this->_num_operacion . $this->_importe . $this->_tipoMoneda . $this->_exponente . $this->_cifrado . $this->_url_ok . $this->_url_nok;
              //    CH8IMEGM.086561966.0000554027.00000003.1619553318.5800.978.2.SHA2.https://viejasdeizan.com/products/checkout/ceca/ok.https://viejasdeizan.com/products/checkout/ceca/ko
              //    CH8IMEGM 086561966 0000554027 00000003 1619553318 5800 978 2 SHA2 https://viejasdeizan.com/products/checkout/ceca/ok https://viejasdeizan.com/products/checkout/ceca/ko
        //$firmaSHA256 =hash('sha256', $firma);
        return $formulario; // .'<p>'.$firmaSHA256.'<br />'.$this->_firma.'<br />'.$firma.'</p>';
 // <input type="text" name="Firma"            value="a739d4e934b89461f8338307d82cac34f37572f0f98740406f7d4b6d34acc54d">
 //                                                   a739d4e934b89461f8338307d82cac34f37572f0f98740406f7d4b6d34acc54d
    }
